...
inline void addToothLogEntry(unsigned long time)
{
	...
	} //The tooth log ready bit is cleared to ensure that we only get a set of concurrent values. 
	else
	{
		toothHistoryIndex++;
	}
...
void triggerPri_missingTooth()
{
	...
	{
		return;
	} //Debounce check. Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
...
int getCrankAngle_DualWheel(int timePerDegree)
{
	...
	{
		tempToothCurrentCount = configPage2.triggerTeeth;
	}
...
void triggerPri_BasicDistributor()
{
	...
	{
		return;
	} //Debounce check. Pulses should never be less than triggerFilterTime
...
int getRPM_BasicDistributor()
{
	...
	{
		return crankingGetRPM(triggerActualTeeth);
	}
	else
	{
		return stdGetRPM();
...
int getCrankAngle_24X(int timePerDegree)
{
	...
	if (toothCurrentCount == 0)
	{
		crankAngle = 0 + configPage2.triggerAngle;
	...
	{
		crankAngle = toothAngles[(tempToothCurrentCount - 1)] + configPage2.triggerAngle;
...
int getCrankAngle_Jeep2000(int timePerDegree)
{
	...
	if (toothCurrentCount == 0)
	{
		crankAngle = 146 + configPage2.triggerAngle;
	} //This is the special case to handle when the 'last tooth' seen was the cam tooth. 146 is the angle at which the crank tooth goes high. 
	else
	{
		crankAngle = toothAngles[(tempToothCurrentCount - 1)] + configPage2.triggerAngle;
	} //Perform a lookup of the fixed toothAngles array to find what the angle of the last tooth passed was. 
