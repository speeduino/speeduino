...
void tableValueMultiply(table_row_iterator &row, table3d_value_t multiplier) {  // cppcheck-suppress [constParameter,constParameterCallback]
  *row = *row * multiplier; 
}

void tableAxisMultiply(table_axis_iterator &axis, table3d_axis_t multiplier) {  // cppcheck-suppress [constParameter,constParameterCallback]
  *axis = *axis * multiplier; 
}

void tableAxisDivide(table_axis_iterator &axis, table3d_axis_t divisor) {  // cppcheck-suppress [constParameter,constParameterCallback]
  *axis = *axis / divisor; 
}
...
void doUpdates(void)
{
  ...
  if (loadEEPROMVersion() == 14U)
  {
    ...
    {
      uint16_t oldAddress = EEPROM_CALIBRATION_CLT_OLD + (x * 16U);
      cltCalibration_values[x] = getStorageAPI().read(oldAddress);
      cltCalibration_bins[x] = (x * 32U);

      oldAddress = EEPROM_CALIBRATION_IAT_OLD + (x * 16U);
      iatCalibration_values[x] = getStorageAPI().read(oldAddress);
      iatCalibration_bins[x] = (x * 32U);

      oldAddress = EEPROM_CALIBRATION_O2_OLD + (x * 16U);
      o2Calibration_values[x] = getStorageAPI().read(oldAddress);
      o2Calibration_bins[x] = (x * 32U);
    }
