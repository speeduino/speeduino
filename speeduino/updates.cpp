...
void doUpdates(void)
{
  ...
  if (loadEEPROMVersion() == 14U)
  {
    ...
    {
      cltCalibrationTable.values[x] = getStorageAPI().read(EEPROM_CALIBRATION_CLT_OLD + (x * 16U));
      cltCalibrationTable.axis[x] = (x * 32U);

      iatCalibrationTable.values[x] = getStorageAPI().read(EEPROM_CALIBRATION_IAT_OLD + (x * 16U));
      iatCalibrationTable.axis[x] = (x * 32U);

      o2CalibrationTable.values[x] = getStorageAPI().read(EEPROM_CALIBRATION_O2_OLD + (x * 16U));
