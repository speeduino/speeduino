  ...
  switch(currentCommand)
  {
    ...
    case 'P': // set the current page
      ...
      if(Serial.available() > 0)
      {
        ...
        else if((currentPage >= 'A') && (currentPage <= 'F')) { currentPage -= 55; }
        serialStatusFlag = SERIAL_INACTIVE;
      }
      break;
...
void send_raw_entity(const page_iterator_t &entity) { Serial.write((byte *)entity.pData, entity.size); }
...
void send_entity(const page_iterator_t &entity)
{
  switch(entity.type)
  {
    case Raw: return send_raw_entity(entity); break;

    case Table: return send_table_entity(entity); break;
