...
#endif
#if defined(PWM_FAN_AVAILABLE)//Regular PWM idle valve control when there is separate timer for PWM fan
{
  if (idle_pwm_state)
  {
...
#else//PWM idle valve control and PWM Fan control in same interrupt, because there is no separate timer for PWM Fan in mega = PWM idle and FAN frequency are same
{
  if ( ((idle_pwm_state == false) || (idle_max_pwm == true)) && ((fan_pwm_state == false) || (fan_max_pwm == true)) )
  {
    ...
    if( (fan_pwm_value > 0) && (fan_max_pwm == false) ) //Don't toggle if at 0%
    {
    ...
    if( (idle_pwm_state == true) && ((idle_pwm_target_value <= fan_pwm_value) || (fan_pwm_state == false))  )
    {
      ...
      if (idle_pwm_target_value == fan_pwm_value) { nextIdleFan = 2; } //Next event is for both PWM
      else { nextIdleFan = 0; } //Next event is for PWM0
    }
    else if( fan_pwm_state == true )
    {
      IDLE_COMPARE = IDLE_COUNTER + fan_pwm_value;
      idle_pwm_cur_value = idle_pwm_target_value;
      fan_pwm_cur_value = fan_pwm_value;
      nextIdleFan = 1; //Next event is for PWM1
    }
    else { IDLE_COMPARE = IDLE_COUNTER + idle_pwm_max_count; } //Shouldn't ever get here
  }
  else
  {
      if(nextIdleFan == 0)
    {
      ...
      nextIdleFan = 1; //Next event is for PWM1
      if(fan_pwm_state == true){ IDLE_COMPARE = IDLE_COUNTER + (fan_pwm_cur_value - idle_pwm_cur_value); }
      else
      { 
        IDLE_COMPARE = IDLE_COUNTER + (idle_pwm_max_count - idle_pwm_cur_value);
        nextIdleFan = 2; //Next event is for both PWM
      }
    }
    else if (nextIdleFan == 1)
    {
      ...
      else { fan_max_pwm = true; }
      nextIdleFan = 0; //Next event is for PWM0
      if(idle_pwm_state == true) { IDLE_COMPARE = IDLE_COUNTER + (idle_pwm_cur_value - fan_pwm_cur_value); }
      else
      { 
        IDLE_COMPARE = IDLE_COUNTER + (idle_pwm_max_count - fan_pwm_cur_value);
        nextIdleFan = 2; //Next event is for both PWM
      }
    }
    else
