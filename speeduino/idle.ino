...
void idleControl()
{
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_PWM_OL:      //Case 2 is PWM open loop
      ...
      if(configPage9.airConIdleSteps>0 && BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true) { currentStatus.idleDuty += configPage9.airConIdleSteps; } // Add air conditioning idle-up
    ...
    case IAC_ALGORITHM_PWM_CL:    //Case 3 is PWM closed loop
      ...
      else
      {
        ...
        if(PID_computed == true)
        {
          ...
          if(configPage9.airConIdleSteps>0 && BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
          {
            ...
            TEMP_idle_pwm_target_value += percentage(configPage9.airConIdleSteps, idle_pwm_max_count<<2);
          ...
          { 
            ...
            // Again, we use configPage9.airConIdleSteps * idle_pwm_max_count / 100 because we are adding percentage steps, but the loop doesn't operate in percentage steps - it works in PWM count
    ...
    case IAC_ALGORITHM_PWM_OLCL: //case 6 is PWM Open Loop table as feedforward term plus closed loop. 
      ...
      else  
      {
        ...
        if(configPage9.airConIdleSteps>0 && BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
        {
          ...
          FeedForwardTerm += percentage(configPage9.airConIdleSteps, (idle_pwm_max_count<<2));
