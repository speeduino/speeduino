...
{
  if((configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_CL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_OL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_OLCL)) { IDLE_TIMER_ENABLE(); }
...
static inline byte checkForStepping(void)
{
  ...
  {
    if(idleStepper.stepperStatus == STEPPING) { timeCheck = iacStepTime_uS; }
    else { timeCheck = iacCoolTime_uS; }
    ...
    if(micros_safe() > (idleStepper.stepStartTime + timeCheck))
    {
      if(idleStepper.stepperStatus == STEPPING)
      {
        ...
        else { idleStepper.stepperStatus = SOFF; }
...
static inline void doStep(void)
{
  if((idleStepper.targetIdleStep <= (idleStepper.curIdleStep - configPage6.iacStepHyster)) || (idleStepper.targetIdleStep >= (idleStepper.curIdleStep + configPage6.iacStepHyster))) // Hysteresis check
  {
    ...
    else if(idleStepper.targetIdleStep > idleStepper.curIdleStep)
    {
...
void idleControl(void)
{
  ...
  switch(configPage6.iacAlgorithm)
  {
    ...
    case IAC_ALGORITHM_PWM_OL: // Case 2 is PWM open loop
      ...
      else
      {
        if(idleTaper < configPage2.idleTaperTime)
        {
          ...
          currentStatus.idleLoad = map(idleTaper, 0, configPage2.idleTaperTime, table2D_getValue(&iacCrankDutyTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET), table2D_getValue(&iacPWMTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET));
    ...
    case IAC_ALGORITHM_PWM_OLCL: // case 6 is PWM Open Loop table as feedforward term plus closed loop.
      ...
      else
      {
        ...
        { // reset integral to zero when TPS is bigger than set value in TS (opening throttle so not idle anymore). OR when RPM higher than Idle Target + RPM Histeresis (coming back from high rpm with throttle closed)
      ...
      if((checkForStepping() == false) && (isStepperHomed() == true)) // Check that homing is complete and that there's not currently a step already taking place. MUST BE IN THIS ORDER!
      {
        ...
        else
        {
          ...
          if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) && (currentStatus.RPM > 0))
          {
            ...
            {
              // Tapering between cranking IAC value and running
              idleStepper.targetIdleStep = map(idleTaper, 0, configPage2.idleTaperTime, table2D_getValue(&iacCrankStepsTable, (currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET)) * 3, table2D_getValue(&iacStepTable, (currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET)) * 3);
              if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ)) { idleTaper++; }
            }
            else
            {
        ...
        else
        {
          if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ))
          {
            ...
            else if(configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL)
            {
              ...
              if(((currentStatus.RPM - idle_cl_target_rpm) > configPage2.iacRPMlimitHysteresis * 10) || (currentStatus.TPS > configPage2.iacTPSlimit) || lastDFCOValue) { idlePID.ResetIntegeral(); }
            }
          ...
          if((currentStatus.TPS > configPage2.iacTPSlimit) || lastDFCOValue || ((configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL) && (idleTaper < configPage2.idleTaperTime))) { idle_pid_target_value = FeedForwardTerm; }
  ...
  if((configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_OL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_CL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_OLCL))
  {
    ...
    else if(currentStatus.idleLoad == 0) { disableIdle(); }
    else
    {
      BIT_SET(currentStatus.spark, BIT_SPARK_IDLE); // Turn the idle control flag on
      IDLE_TIMER_ENABLE();
