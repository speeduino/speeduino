...
{
  noInterrupts();
  bool updatedRevTime = HasAnySync(currentStatus) && !IsCranking(currentStatus) && (toothOneMinusOneTime != UINT32_C(0)) && (toothOneTime > toothOneMinusOneTime)
...
static inline void setFilter(unsigned long curGap)
{
  ...
  else if (configPage4.triggerFilter == 1)
  {
...
void triggerSetEndTeeth_missingTooth(void)
{
  ...
  if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) && (configPage4.TrigSpeed == CRANK_SPEED) && (configPage2.strokes == FOUR_STROKE))
  {
...
void triggerSec_DualWheel(void)
{
  ...
  if (curGap2 >= triggerSecFilterTime)
  {
    ...
    {
      if ((toothCurrentCount != configPage4.triggerTeeth) && (currentStatus.startRevolutions > 2))
      {
        currentStatus.syncLossCounter++;
...
void triggerSetup_BasicDistributor(void)
{
  ...
  } // Minimum 90rpm. (1851uS is the time per degree at 90rpm). This uses 90rpm rather than 50rpm due to the potentially very high stall time on a 4 cylinder if we wait that long.
  else
  {
    MAX_STALL_TIME = ((MICROS_PER_DEG_1_RPM / 50U) * triggerToothAngle);
  } // Minimum 50rpm. (3200uS is the time per degree at 50rpm).
}
...
void triggerPri_BasicDistributor(void)
{
  ...
  if ((curGap >= triggerFilterTime))
  {
    ...
    {
      triggerFilterTime = 0;
    } // If we don't yet have sync, ensure that the filter won't prevent future valid pulses from being ignored.
    ...
    else
    {
      ...
      else
      {
        if ((READ_SEC_TRIGGER() == false) && (revolutionOne == true))
        {
          ...
          {
            toothCurrentCount = 1;
          } // Means we're at 5* BTDC on a 4G63 4 cylinder
...
void triggerPri_24X(void)
{
  ...
  {
    currentStatus.hasSync = false;
  } // Indicates sync has not been achieved (Still waiting for 1 revolution of the crank to take place)
  else
  {
    curTime = micros();
    curGap = curTime - toothLastToothTime;
...
int getCrankAngle_24X(void)
{
  ...
  } // This is the special case to handle when the 'last tooth' seen was the cam tooth. 0 is the angle at which the crank tooth goes high (Within 360 degrees).
  else
  {
...
int getCrankAngle_Jeep2000(void)
{
  ...
  } // This is the special case to handle when the 'last tooth' seen was the cam tooth. Since  the tooth timings were taken on the previous crank tooth, the previous crank tooth angle is used here, not cam angle.
  else
  {
    crankAngle = toothAngles[(tempToothCurrentCount - 1)] + configPage4.triggerAngle;
  } // Perform a lookup of the fixed toothAngles array to find what the angle of the last tooth passed was.
...
void triggerSec_Nissan360(void)
{
  ...
  else
  {
    ...
    if (currentStatus.hasSync == false)
    {
      ...
      {
        currentStatus.hasSync = false;
      } // This should really never happen (Only 4, 6 and 8 cylinder engines for this pattern)
    }
    else
    {
...
void triggerPri_Subaru67(void)
{
  ...
  if (currentStatus.hasSync == true)
  {
    ...
    } // Special case for tooth 1
    else if (toothCurrentCount == 2)
    {
      triggerToothAngle = 93;
    } // Special case for tooth 2
    else
    {
...
void triggerSetEndTeeth_ThirtySixMinus222(void)
{
  ...
  else if (configPage2.nCylinders == 6)
  {
    ...
    if (currentStatus.advance < 10)
    {
      ignition3EndTooth = 23;
    }
    else if (currentStatus.advance < 20)
    {
      ignition3EndTooth = 22;
    }
    else if (currentStatus.advance < 30)
    {
      ignition3EndTooth = 21;
    }
    else if (currentStatus.advance < 40)
    {
      ignition3EndTooth = 20;
    }
    else
    {
      ignition3EndTooth = 19;
    }
  }
}
/** @} */
...
void triggerPri_ThirtySixMinus21(void)
{
  ...
  if (curGap >= triggerFilterTime) // Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
  {
    ...
    targetGap2 = ((5 * (toothLastToothTime - toothLastMinusOneToothTime)) >> 1); // check for 2.5 gap
    targetGap = ((3 * (toothLastToothTime - toothLastMinusOneToothTime)) >> 1); // Multiply by 1.5 (Checks for a gap 1.5x greater than the last one) (Uses bitshift to divide by 2 as in the missing tooth decoder)
...
void triggerPri_Webber(void)
{
  ...
  if (curGap >= triggerFilterTime)
  {
    ...
    if (triggerSecFilterTime <= curGap)
    {
    ...
    if (toothLastSecToothTime > 0 && toothLastMinusOneSecToothTime > 0) // Make sure we have enough tooth information to calculate tooth lengths
    {
      ...
      if (curGap2 >= ((3 * (toothLastSecToothTime - toothLastMinusOneSecToothTime)) >> 1)) // Check if we have a bigger gap, that is a long tooth
      {
        ...
        if ((toothLastSecToothRisingTime - toothLastSecToothTime) < (curTime2 - toothLastSecToothRisingTime))
        {
          ...
          {
            secondaryToothCount = 0;
          } // Any other number of teeth seen means we missed something or something extra was seen so attempt resync.
        }
        else
        {
          // Just passed the first tooth after the LOW missing tooth
...
void triggerSetup_Vmax(void)
{
  ...
  {
    toothLastToothTime = micros();
  } // Set a startup value here to avoid filter errors when starting. This MUST have the initi check to prevent the fuel pump just staying on all the time
  triggerFilterTime = 1500;
...
void triggerPri_Renix(void)
{
  ...
  if (curGap >= triggerFilterTime)
  {
    ...
    } // random large number to stop system thinking we have a gap for the first few teeth on start up
...
void triggerSetEndTeeth_RoverMEMS()
{
  ...
  if (tempIgnitionEndTooth[1] > (36 + toothAdder))
  {
    tempIgnitionEndTooth[1] -= (36 + toothAdder);
  }
  ...
  if (tempIgnitionEndTooth[2] > (36 + toothAdder))
  {
    tempIgnitionEndTooth[2] -= (36 + toothAdder);
  }
  ...
  if (tempIgnitionEndTooth[3] > (36 + toothAdder))
  {
    tempIgnitionEndTooth[3] -= (36 + toothAdder);
  }
  ...
  if (tempIgnitionEndTooth[4] > (36 + toothAdder))
  {
    tempIgnitionEndTooth[4] -= (36 + toothAdder);
  }
