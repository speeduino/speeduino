...
static constexpr inline bool _canApply8BitDiv(uint16_t working, uint16_t scale) {
    return ((scale < (uint16_t)UINT8_MAX) && udiv_is8bit_result((uint16_t)working, (uint8_t)scale));
}
                ...
                (factor.scale==2U) ? working >> 1U :
                    ...
                    (factor.scale==100U) ? div100(working) :
                        ...
                        (factor.scale==10U) ? (working/10U) :
                            ...
                            _canApply8BitDiv(working, factor.scale) ? (uint16_t)udiv_16_8((uint16_t)working, (uint8_t)factor.scale) :
                                // Slowest path
                                working = working / factor.scale;
}
