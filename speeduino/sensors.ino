...
void initialiseADC(void)
{
  ...
  for(byte AuxinChan = 0; AuxinChan < 16; AuxinChan++)
  {
    ...
    if(((configPage9.caninput_sel[currentStatus.current_caninchannel] & 12) == 4) && ((configPage9.enable_secondarySerial == 1) || ((configPage9.enable_intcan == 1) && (configPage9.intcan_available == 1))))
...
// Instantaneous MAP readings
  ...
  if(configPage6.useEMAP == true)
  {
    ...
    // Error check
    if((tempReading < VALID_MAP_MAX) && (tempReading > VALID_MAP_MIN)) { currentStatus.EMAPADC = ADC_FILTER(tempReading, configPage4.ADCFILTER_MAP, currentStatus.EMAPADC); }
  ...
  switch(configPage2.mapSample)
  {
    ...
    case 1:
      ...
      else
      {
        ...
        if(configPage6.useEMAP == true) { EMAPrunningValue = currentStatus.EMAPADC; }
        MAPcount = 1;
      }
      break;
    ...
    case 3:
      ...
      if((currentStatus.RPMdiv100 > configPage2.mapSwitchPoint) && ((currentStatus.hasSync == true) || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)) && (currentStatus.startRevolutions > 1) &&
...
void readBat(void)
{
  ...
  if(tempReading < 0) { tempReading = 0; } // with negative overflow prevention
  ...
  if((currentStatus.battery10 < 55) && (tempReading > 70) && (currentStatus.RPM == 0))
  {
    ...
    if((configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_CL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OL)) { initialiseIdle(true); }
  }
...
uint16_t getSpeed(void)
{
  ...
  if(configPage2.vssMode == 1)
  {
    ...
    if(configPage2.vssPulsesPerKm == 0) { tempSpeed = currentStatus.canin[configPage2.vssAuxCh]; }
    // Adjust the reading by dividing it by set amount.
    else { tempSpeed = (currentStatus.canin[configPage2.vssAuxCh] / configPage2.vssPulsesPerKm); }
