...
static inline bool isTimeout(void) { return (millis() - serialReceiveStartTime) > SERIAL_TIMEOUT; }
...
static __attribute__((noinline)) uint32_t readSerial32Timeout(void)
{
  ...
  {
    if(Serial.available() != 0) { buffer.raw[count++] = (byte)Serial.read(); }
    else if(isTimeout()) { return 0; }
    else
    { /* MISRA - no-op */
    }
  }
...
{
  if(start < length) { start = start + writeNonBlocking(buffer + start, length - start); }

  if(start >= length && start < length + sizeof(crc_t)) { start = start + writeNonBlocking(start - length, CRC32_serial.crc32(buffer, length)); }
...
void serialTransmit(void)
{
  ...
  {
    case SERIAL_TRANSMIT_INPROGRESS_LEGACY: sendValues(logItemsTransmitted, inProgressLength, SEND_OUTPUT_CHANNELS, 0); break;

    case SERIAL_TRANSMIT_TOOTH_INPROGRESS: sendToothLog(); break;

    case SERIAL_TRANSMIT_TOOTH_INPROGRESS_LEGACY: sendToothLog_legacy(logItemsTransmitted); break;

    case SERIAL_TRANSMIT_COMPOSITE_INPROGRESS: sendCompositeLog(); break;
...
void processSerialCommand(void)
{
  switch(serialPayload[0])
  {
    ...
    case 'M': {
      ...
      // 2 - Length
      // 1 - 1st New value
      if(updatePageValues(serialPayload[2], word(serialPayload[4], serialPayload[3]), &serialPayload[7], word(serialPayload[6], serialPayload[5]))) { sendReturnCodeMsg(SERIAL_RC_OK); }
      else
      {
    ...
    case 't': // receive new Calibration info. Command structure: "t", <tble_idx> <data array>.
    {
      ...
      else if(cmd == IAT_CALIBRATION_PAGE) { processTemperatureCalibrationTableUpdate(calibrationLength, IAT_CALIBRATION_PAGE, iatCalibration_values, iatCalibration_bins); }
      else if(cmd == CLT_CALIBRATION_PAGE) { processTemperatureCalibrationTableUpdate(calibrationLength, CLT_CALIBRATION_PAGE, cltCalibration_values, cltCalibration_bins); }
      else { sendReturnCodeMsg(SERIAL_RC_RANGE_ERR); }
      break;
    }
