...
unsigned long MAX_STALL_TIME =
    500000UL; // The maximum time (in uS) that the system will continue to function before the engine is considered stalled/stopped. This is unique to each decoder, depending on the number of teeth etc. 500000 (half a second) is used as the default value, most decoders will be much less.
...
static inline void addToothLogEntry(unsigned long toothTime, byte whichTooth)
{
  ...
  if((currentStatus.toothLogEnabled == true) || (currentStatus.compositeTriggerUsed > 0))
  {
    ...
    else if(currentStatus.compositeTriggerUsed > 0)
    {
      ...
      if(revolutionOne == 1) { BIT_SET(compositeLogHistory[toothHistoryIndex], COMPOSITE_ENGINE_CYCLE); }
      else { BIT_CLEAR(compositeLogHistory[toothHistoryIndex], COMPOSITE_ENGINE_CYCLE); }
...
void loggerSecondaryISR(void)
{
  ...
  */
  if(((secondaryTriggerEdge == RISING) && (READ_SEC_TRIGGER() == HIGH)) || ((secondaryTriggerEdge == FALLING) && (READ_SEC_TRIGGER() == LOW)) || (secondaryTriggerEdge == CHANGE)) { triggerSecondaryHandler(); }
...
void loggerTertiaryISR(void)
{
  ...
  */
  ...
  if(((tertiaryTriggerEdge == RISING) && (READ_THIRD_TRIGGER() == HIGH)) || ((tertiaryTriggerEdge == FALLING) && (READ_THIRD_TRIGGER() == LOW)) || (tertiaryTriggerEdge == CHANGE)) { triggerTertiaryHandler(); }
  // No tooth logger for the secondary input
  if((currentStatus.compositeTriggerUsed > 0) && (BIT_CHECK(decoderState, BIT_DECODER_VALID_TRIGGER)))
  {
    // Composite logger adds an entry regardless of which edge it was
...
static inline int crankingGetRPM(byte totalTeeth, uint16_t degreesOver)
{
  ...
  if((currentStatus.startRevolutions >= configPage4.StgCycles) && ((currentStatus.hasSync == true) || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)))
  {
    if((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0) && (toothLastToothTime > toothLastMinusOneToothTime))
    {
      ...
      } // Sanity check. This can prevent spiking caused by noise on individual teeth. The new RPM should never be above 4x the cranking setting value (Remembering that this function is only called is the current RPM is less than the cranking setting)
    }
  }
...
static inline void checkPerToothTiming(int16_t crankAngle, uint16_t currentTooth)
{
  if((fixedCrankingOverride == 0) && (currentStatus.RPM > 0))
  {
    ...
    else if((currentTooth == ignition2EndTooth))
    {
      ...
      {
        ignitionSchedule2.endCompare              = IGN2_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition2EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition3EndTooth))
    {
      ...
      {
        ignitionSchedule3.endCompare              = IGN3_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition3EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition4EndTooth))
    {
      ...
      {
        ignitionSchedule4.endCompare              = IGN4_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition4EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition5EndTooth))
    {
      ...
      {
        ignitionSchedule5.endCompare              = IGN5_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition5EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition6EndTooth))
    {
      ...
      {
        ignitionSchedule6.endCompare              = IGN6_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition6EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition7EndTooth))
    {
      ...
      {
        ignitionSchedule7.endCompare              = IGN7_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition7EndAngle - crankAngle))));
    ...
    else if((currentTooth == ignition8EndTooth))
    {
      ...
      {
        ignitionSchedule8.endCompare              = IGN8_COUNTER + uS_TO_TIMER_COMPARE(fastDegreesToUS(ignitionLimits((ignition8EndAngle - crankAngle))));
...
void triggerSetup_missingTooth(void)
{
  ...
  if(configPage4.trigPatternSec == SEC_TRIGGER_4_1) { triggerSecFilterTime = 1000000 * 60 / MAX_RPM / 4 / 2; }
  else { triggerSecFilterTime = (1000000 / (MAX_RPM / 60)); }
...
void triggerSetEndTeeth_missingTooth(void)
{
  byte toothAdder  = 0;
  byte toothMargin = 0;
  if(configPage4.triggerTeeth > 12) { toothMargin = 1; } // For higher tooth count triggers, add a 1 tooth margin to allow for calculation time.
  ...
  tempIgnition1EndTooth = ((ignition1EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition2EndTooth = ((ignition2EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition3EndTooth = ((ignition3EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition4EndTooth = ((ignition4EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition5EndTooth = ((ignition5EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition6EndTooth = ((ignition6EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition7EndTooth = ((ignition7EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
  ...
  tempIgnition8EndTooth = ((ignition8EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle)) - toothMargin;
...
{
  uint16_t tempRPM;
  if(currentStatus.RPM < currentStatus.crankRPM || currentStatus.RPM < 1500) { tempRPM = crankingGetRPM(triggerActualTeeth, 720); }
...
void triggerPri_GM7X(void)
{
  ...
  if((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0))
  {
    ...
    else
    {
      ...
      else { BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT); }
    }
  }
...
uint16_t getRPM_GM7X(void) { return stdGetRPM(360); }
int      getCrankAngle_GM7X(void)
{
  ...
  else if(tempToothCurrentCount == 3) { crankAngle = 112; }
  else
  {
      ...
      else
      {
        ...
        else if((READ_SEC_TRIGGER() == true) && (revolutionOne == true))
        {
          ...
          else if(configPage2.nCylinders == 6)
          {
            toothCurrentCount     = 2;
            currentStatus.hasSync = true;
          } // Means we're at 45* ATDC on 6G72 6 cylinder
        }
      }
    }
  } // Filter time
...
void triggerSetup_24X(void)
{
  ...
  if(initialisationComplete == false)
  {
    ...
    toothLastToothTime = micros();
  } // Set a startup value here to avoid filter errors when starting. This MUST have the init check to prevent the fuel pump just staying on all the time
  BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
  BIT_SET(decoderState, BIT_DECODER_IS_SEQUENTIAL);
  BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT);
  BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
...
uint16_t getRPM_24X(void) { return stdGetRPM(360); }
...
void triggerSetEndTeeth_24X(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
void triggerSetup_Jeep2000(void)
{
  ...
  if(initialisationComplete == false)
  {
    ...
    toothLastToothTime = micros();
  } // Set a startup value here to avoid filter errors when starting. This MUST have the initial check to prevent the fuel pump just staying on all the time
  BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
  BIT_CLEAR(decoderState, BIT_DECODER_IS_SEQUENTIAL);
  BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT);
  BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
...
uint16_t getRPM_Jeep2000(void) { return stdGetRPM(360); }
...
void triggerSetEndTeeth_Jeep2000(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
uint16_t getRPM_Audi135(void) { return stdGetRPM(360); }
...
void triggerSetEndTeeth_Audi135(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
void triggerPri_HondaD17(void)
{
  ...
  //
  if((toothCurrentCount == 13) && (currentStatus.hasSync == true)) { toothCurrentCount = 0; }
...
uint16_t getRPM_HondaD17(void) { return stdGetRPM(360); }
...
void triggerSetEndTeeth_HondaD17(void) { lastToothCalcAdvance = currentStatus.advance; }
  ...
  if((curGap >= triggerFilterTime) || (currentStatus.startRevolutions == 0))
  {
    ...
    if(currentStatus.hasSync == true)
    {
      ...
      if((configPage4.triggerFilter == 1) || (currentStatus.RPM < 1400))
      {
        ...
        } // Trigger filter is set to whatever time it took to do 70 degrees (Next trigger is 110 degrees away)
        else
        {
          triggerToothAngle = 110;
          triggerFilterTime = (curGap * 3) >> 3;
        } // Trigger filter is set to (110*3)/8=41.25=41 degrees (Next trigger is 70 degrees away).
      }
      else if(configPage4.triggerFilter == 2)
      {
        // Medium filter level
        ...
        } // 87.5 degrees with a target of 110
        else
        {
          triggerToothAngle = 110;
          triggerFilterTime = (curGap >> 1);
        } // 55 degrees with a target of 70
      }
      else if(configPage4.triggerFilter == 3)
      {
        // Aggressive filter level
        ...
        } // 96.26 degrees with a target of 110
        else
        {
          triggerToothAngle = 110;
          triggerFilterTime = (curGap * 9) >> 5;
        } // 61.87 degrees with a target of 70
      }
      ...
      if((configPage2.perToothIgn == true) && (configPage4.triggerAngle == 0) && (currentStatus.advance > 0))
      {
        int16_t crankAngle = ignitionLimits(toothAngles[(toothCurrentCount - 1)]);
...
void triggerSec_Miata9905(void)
{
  ...
  if(curGap2 >= triggerSecFilterTime)
  {
    ...
    if((toothCurrentCount == 1) && (curTime2 > toothLastToothTime)) { lastVVTtime = curTime2 - toothLastToothTime; }
  }
}
  ...
  if(curGap >= triggerFilterTime)
  {
    ...
    if(currentStatus.hasSync == true)
    {
      ...
      if((toothCurrentCount == 1) || (toothCurrentCount == 3))
      {
        ...
        triggerFilterTime = curGap;
      } // Trigger filter is set to whatever time it took to do 72 degrees (Next trigger is 108 degrees away)
      else
      {
        triggerToothAngle = 108;
        triggerFilterTime = (curGap * 3) >> 3;
      } // Trigger filter is set to (108*3)/8=40 degrees (Next trigger is 70 degrees away).
...
void triggerSec_MazdaAU(void)
{
  ...
  if(currentStatus.hasSync == false)
  {
    ...
    else
    {
      ...
      targetGap         = (lastGap) >> 1; // The target gap is set at half the last tooth gap
      if(curGap2 <
         targetGap) // If the gap between this tooth and the last one is less than half of the previous gap, then we are very likely at the extra (3rd) tooth on the cam). This tooth is located at 421 crank degrees (aka 61 degrees) and therefore the last crank tooth seen was number 1 (At 350 degrees)
      {
...
void triggerSetEndTeeth_MazdaAU(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
void triggerSetEndTeeth_non360(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
void triggerPri_Nissan360(void)
{
  ...
  if(currentStatus.hasSync == true)
  {
    ...
    ; // The time per crank degree is simply the time between this tooth and the last one divided by 2
  }
}
...
void triggerPri_Subaru67(void)
{
  ...
  switch(secondaryToothCount)
  {
    ...
    case 1:
      ...
      if(toothCurrentCount == 5 || toothCurrentCount == 11) { currentStatus.hasSync = true; }
      else
      {
        currentStatus.hasSync = false;
        currentStatus.syncLossCounter++;
    ...
    case 3:
      if(toothCurrentCount == 2) { currentStatus.hasSync = true; }
      else
      {
        currentStatus.hasSync = false;
        currentStatus.syncLossCounter++;
...
uint16_t getRPM_Daihatsu(void)
{
  ...
  else { tempRPM = stdGetRPM(720); } // Tracking over 2 crank revolutions
...
void triggerSetEndTeeth_Daihatsu(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
int getCrankAngle_Harley(void)
{
  ...
  else { crankAngle = 157 + configPage4.triggerAngle; }
...
void triggerSetEndTeeth_Harley(void) { lastToothCalcAdvance = currentStatus.advance; }
/** @} */
...
uint16_t getRPM_ThirtySixMinus222(void)
{
  ...
  if(currentStatus.RPM < currentStatus.crankRPM)
  {
    ...
    if((configPage2.nCylinders == 4) && (toothCurrentCount != 19) && (toothCurrentCount != 16) && (toothCurrentCount != 34) && (BIT_CHECK(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT))) { tempRPM = crankingGetRPM(36, 360); }
    else if((configPage2.nCylinders == 6) && (toothCurrentCount != 9) && (toothCurrentCount != 12) && (toothCurrentCount != 33) && (BIT_CHECK(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT))) { tempRPM = crankingGetRPM(36, 360); }
...
uint16_t getRPM_ThirtySixMinus21(void)
{
  ...
  if(currentStatus.RPM < currentStatus.crankRPM)
  {
    if((toothCurrentCount != 20) && (BIT_CHECK(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT))) { tempRPM = crankingGetRPM(36, 360); }
    else { tempRPM = currentStatus.RPM; } // Can't do per tooth RPM if we're at tooth #1 as the missing tooth messes the calculation
  }
  else { tempRPM = stdGetRPM(360); }
  return tempRPM;
...
void triggerSetup_FordST170(void)
{
  ...
  triggerSecFilterTime = 1000000 * 60 / MAX_RPM / 8 / 2; // Cam pattern is 8-3, so 2 nearest teeth are 90 deg crank angle apart. Cam can be advanced by 60 deg, so going from fully retarded to fully advanced closes the gap to 30 deg. Zetec cam pulleys aren't keyed from factory, so I subtracted
                                                         // additional 10 deg to avoid filter to be too aggressive. And there you have it 720/20=36.
...
uint16_t getRPM_FordST170(void)
{
  ...
  if(currentStatus.RPM < currentStatus.crankRPM)
  {
    if(toothCurrentCount != 1) { tempRPM = crankingGetRPM(36, 360); }
    else { tempRPM = currentStatus.RPM; } // Can't do per tooth RPM if we're at tooth #1 as the missing tooth messes the calculation
  }
  else { tempRPM = stdGetRPM(360); }
  return tempRPM;
...
void triggerPri_NGC(void)
{
  ...
  if(curGap >= triggerFilterTime) // Pulses should never be less than triggerFilterTime, so if they are it means a false trigger.
  {
    ...
    { // If we have a missing tooth, copy the gap from the previous tooth as that is the correct normal tooth length
      toothLastMinusOneToothTime = curTime - (toothLastToothTime - toothLastMinusOneToothTime);
    }
    else { toothLastMinusOneToothTime = toothLastToothTime; }
    toothLastToothTime = curTime;
...
void triggerSec_NGC4(void)
{
  ...
  if(configPage4.sparkMode != IGN_MODE_SEQUENTIAL && configPage2.injLayout != INJ_SEQUENTIAL) { return; }
    ...
    if(toothLastSecToothTime > 0 && toothLastToothTime > 0 && toothLastMinusOneToothTime > 0) // Make sure we have enough tooth information to calculate tooth lengths
    {
      ...
      if(curGap2 >= (3 * (toothLastToothTime - toothLastMinusOneToothTime))) // Check if we have a bigger gap, that is missing teeth
      {
        ...
        { // Only check for cam sync if we have actually detected two groups and can get cam sync
          if(toothSystemCount > 0 && secondaryToothCount == (unsigned int)toothAngles[toothSystemCount + 1])
          { // Do a quick check if we already have cam sync
            toothSystemCount++;
            if(toothSystemCount > configPage2.nCylinders) { toothSystemCount = 1; }
          }
          else
          {                       // Check for a pair of matching groups which tells us which group we are at, this should only happen when we don't have cam sync
            ...
            {
              if(secondaryToothCount == (unsigned int)toothAngles[group] && secondaryToothLastCount == (byte)toothAngles[group - 1])
...
// We measure the width of a lobe so on the end of a lobe, but want to trigger on the beginning. Variable toothCurrentCount tracks the downward events, and secondaryToothCount updates on the upward events. Ideally, it should be the other way round but the engine stall routine resets
// secondaryToothCount, so it would not sync again after an engine stall.

void triggerPri_Vmax(void)
{
  curTime = micros();
  if(READ_PRI_TRIGGER() == primaryTriggerEdge)
  { // Forwarded from the config page to setup the primary trigger edge (rising or falling). Inverting VR-conditioners require FALLING, non-inverting VR-conditioners require RISING in the Trigger edge setup.
    curGap2 = curTime;
    curGap  = curTime - toothLastToothTime;
    if((curGap >= triggerFilterTime))
    {
      ...
      if(toothCurrentCount > 0)                         // We have sync based on the tooth width.
      {
        ...
        toothLastMinusOneToothTime = toothLastToothTime;
        toothLastToothTime         = curTime;
        if(triggerFilterTime > 50000)
        { // The first pulse seen
          triggerFilterTime = 0;
        }
      }
      else
  ...
  else if(BIT_CHECK(decoderState, BIT_DECODER_VALID_TRIGGER)) // Inverted due to vr conditioner. So this is the falling lobe. We only process if there was a valid trigger.
  {
    ...
    if(curGap3 > (lastGap * 2))
    { // Small lobe is 5 degrees, big lobe is 45 degrees. So this should be the wide lobe.
      if(toothCurrentCount == 0 || toothCurrentCount == 6)
      { // Wide should be seen with toothCurrentCount = 0, when there is no sync yet, or toothCurrentCount = 6 when we have done a full revolution.
        currentStatus.hasSync = true;
      }
      else
...
void triggerSetEndTeeth_Vmax(void) { lastToothCalcAdvance = currentStatus.advance; }
...
void triggerPri_Renix(void)
{
  ...
  if(curGap >= triggerFilterTime)
  {
    ...
    if(renixSystemLastToothTime != 0 && renixSystemLastMinusOneToothTime != 0) { targetGap = (2 * (renixSystemLastToothTime - renixSystemLastMinusOneToothTime)); } // in real world the physical 2 tooth gap is bigger than 2 teeth - more like 2.5
    else { targetGap = 100000000L; }                                                                                                                                // random large number to stop system thinking we have a gap for the first few teeth on start up
  ...
  if(curGap >= triggerFilterTime) // Pulses should never be less than triggerFilterTime, so if they are it means a false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
  {
    if((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0)) // have we seen more than 1 tooth so we start processing
    {
      ...
      {                                                              // 12345678901234567890123456789012
        ...
        {
          currentStatus.hasSync = false;
          if(secondaryToothCount > 0) { BIT_SET(currentStatus.status3, BIT_STATUS3_HALFSYNC); }
          else { BIT_CLEAR(currentStatus.status3, BIT_STATUS3_HALFSYNC); }
          currentStatus.syncLossCounter++;
        }
      }
    }
...
void triggerSec_RoverMEMS()
{
  ...
  if(curGap2 >= triggerSecFilterTime)
  {
    ...
    // Record the VVT Angle
    if(configPage6.vvtEnabled > 0 && ((configPage4.trigPatternSec == SEC_TRIGGER_SINGLE) || (configPage4.trigPatternSec == SEC_TRIGGER_5_3_2 && secondaryToothCount == 6)))
    {
      int16_t curAngle;
      curAngle = getCrankAngle();
      while(curAngle > 360)
...
uint16_t getRPM_RoverMEMS()
{
  ...
  if(currentStatus.RPM < currentStatus.crankRPM)
  {
    if((toothCurrentCount != (unsigned int)toothAngles[SKIP_TOOTH1]) && (toothCurrentCount != (unsigned int)toothAngles[SKIP_TOOTH2]) && (toothCurrentCount != (unsigned int)toothAngles[SKIP_TOOTH3]) && (toothCurrentCount != (unsigned int)toothAngles[SKIP_TOOTH4]))
    {
      tempRPM = crankingGetRPM(36, 360);
    }
    else { tempRPM = currentStatus.RPM; } // Can't do per tooth RPM as the missing tooth messes the calculation
  }
  else { tempRPM = stdGetRPM(360); }
  return tempRPM;
}
...
void triggerSetEndTeeth_RoverMEMS()
{
  ...
  if(configPage4.sparkMode == IGN_MODE_SEQUENTIAL)
  {
    ...
    {
      if(tempIgnitionEndTooth[tempCount] == (toothAngles[1]) || tempIgnitionEndTooth[tempCount] == (toothAngles[2]) || tempIgnitionEndTooth[tempCount] == (toothAngles[3]) || tempIgnitionEndTooth[tempCount] == (toothAngles[4]) || tempIgnitionEndTooth[tempCount] == (36 + toothAngles[1]) ||
         tempIgnitionEndTooth[tempCount] == (36 + toothAngles[2]) || tempIgnitionEndTooth[tempCount] == (36 + toothAngles[3]) || tempIgnitionEndTooth[tempCount] == (36 + toothAngles[4]))
      {
  ...
  else
  {
    for(tempCount = 1; tempCount < 5; tempCount++)
    {
      if(tempIgnitionEndTooth[tempCount] == (toothAngles[1]) || tempIgnitionEndTooth[tempCount] == (toothAngles[2])) { tempIgnitionEndTooth[tempCount]--; }
    }
  }
