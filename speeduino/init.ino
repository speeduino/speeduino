 ...
 * Workaround for: https://github.com/tonton81/FlexCAN_T4/issues/14 */
  ...
  triggerFilterTime =
      0; // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth (ie at max RPM). Any pulses that occur faster than this time will be discarded as noise. This is simply a default value, the actual values are set in the setup() functions of each decoder
  ...
  if(configPage2.strokes == FOUR_STROKE)
  {
    ...
    req_fuel_uS = req_fuel_uS / 2; // The req_fuel calculation above gives the total required fuel (At VE 100%) in the full cycle. If we're doing more than 1 squirt per cycle then we need to split the amount accordingly. (Note that in a non-sequential 4-stroke setup you cannot have less than 2
                                   // squirts as you cannot determine the stroke to make the single squirt on)
  }
...
// And for inputs
  ...
  if((configPage10.fuelPressureEnable > 0) && (!pinIsOutput(pinFuelPressure))) { pinMode(pinFuelPressure, INPUT); }
  if((configPage10.oilPressureEnable > 0) && (!pinIsOutput(pinOilPressure))) { pinMode(pinOilPressure, INPUT); }
  if((configPage13.onboard_log_trigger_Epin > 0) && (!pinIsOutput(pinSDEnable))) { pinMode(pinSDEnable, INPUT); }
  ...
  if((pinAirConComp > 0) && ((configPage15.airConEnable & 1) == 1)) { pinMode(pinAirConComp, OUTPUT); }
  ...
  if((pinAirConFan > 0) && ((configPage15.airConEnable & 1) == 1) && ((configPage15.airConFanEnabled & 1) == 1)) { pinMode(pinAirConFan, OUTPUT); }
...
void initialiseTriggers(void)
{
  ...
  switch(pinTrigger)
  {
    ...
    case 2: triggerInterrupt = 0; break;
    case 3: triggerInterrupt = 1; break;
    case 18: triggerInterrupt = 5; break;
    case 19: triggerInterrupt = 4; break;
    case 20: triggerInterrupt = 3; break;
    case 21: triggerInterrupt = 2; break;
    default: triggerInterrupt = 0; break; // This should NEVER happen
  }
#else
  ...
  {
    // Arduino Mega 2560 mapping
    case 2: triggerInterrupt2 = 0; break;
    case 3: triggerInterrupt2 = 1; break;
    case 18: triggerInterrupt2 = 5; break;
    case 19: triggerInterrupt2 = 4; break;
    case 20: triggerInterrupt2 = 3; break;
    case 21: triggerInterrupt2 = 2; break;
    default: triggerInterrupt2 = 0; break; // This should NEVER happen
  }
#else
  ...
  {
    // Arduino Mega 2560 mapping
    case 2: triggerInterrupt3 = 0; break;
    case 3: triggerInterrupt3 = 1; break;
    case 18: triggerInterrupt3 = 5; break;
    case 19: triggerInterrupt3 = 4; break;
    case 20: triggerInterrupt3 = 3; break;
    case 21: triggerInterrupt3 = 2; break;
    default: triggerInterrupt3 = 0; break; // This should NEVER happen
  }
#else
...
void changeHalfToFullSync(void)
{
  ...
  noInterrupts();
  if((configPage2.injLayout == INJ_SEQUENTIAL) && (CRANK_ANGLE_MAX_INJ != 720) && fuelSchedule1.Status != RUNNING && fuelSchedule2.Status != RUNNING && fuelSchedule3.Status != RUNNING && fuelSchedule4.Status != RUNNING && fuelSchedule5.Status != RUNNING && fuelSchedule6.Status != RUNNING &&
     fuelSchedule7.Status != RUNNING && fuelSchedule8.Status != RUNNING)
  {
    CRANK_ANGLE_MAX_INJ = 720;
    req_fuel_uS *= 2;
    ...
    switch(configPage2.nCylinders)
    {
      case 4: maxInjOutputs = 4; break;

      case 6: maxInjOutputs = 6; break;

      case 8: maxInjOutputs = 8; break;

      default: break; // No actions required for other cylinder counts
    }
  }
  interrupts();
