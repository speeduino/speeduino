...
void EEPROM_update(uint32_t EEPROM_address, uint8_t data)
{
  byte result = EEPROM_read(EEPROM_address);
  if(result != data) { EEPROM_write(EEPROM_address, data); }
}
...
void writeConfig_STM()
{
  ...
  */
  ...
  //if(EEPROM.read(0) != data_structure_version) { EEPROM.write(,data_structure_version); }   //Write the data structure version
  EEPROM_update(0, data_structure_version); //Write the data structure version
  
  /*---------------------------------------------------
  | Fuel table (See storage.h for data layout) - Page 1
  | 16x16 table itself + the 16 values along each of the axis
  -----------------------------------------------------*/
  EEPROM_update(EEPROM_CONFIG1_XSIZE, fuelTable.xSize); //Write the VE Tables RPM dimension size
  EEPROM_update(EEPROM_CONFIG1_YSIZE, fuelTable.ySize); //Write the VE Tables MAP/TPS dimension size

  for(int x=EEPROM_CONFIG1_MAP; x<EEPROM_CONFIG1_XBINS; x++)
  {
    EEPROM_update(x, fuelTable.values[15-offset/16][offset%16]); //Write the 16x16 map
    offset++;
  }
  //RPM bins
  for(int x=EEPROM_CONFIG1_XBINS; x<EEPROM_CONFIG1_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG1_XBINS;
    EEPROM_update(x, byte(fuelTable.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG1_YBINS; x<EEPROM_CONFIG2_START; x++)
  {
    offset = x - EEPROM_CONFIG1_YBINS;
    EEPROM_update(x, fuelTable.axisY[offset] / TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
  }
  //That concludes the writing of the VE table
  //*********************************************************************************************************************************************************************************
  ...
  for(int x=EEPROM_CONFIG2_START; x<EEPROM_CONFIG2_END; x++)
  {
    EEPROM_update(x, *(pnt_configPage + byte(x - EEPROM_CONFIG2_START)));
  }
  //*********************************************************************************************************************************************************************************
  ...
  -----------------------------------------------------*/
  //Begin writing the Ignition table, basically the same thing as above
  EEPROM_update(EEPROM_CONFIG3_XSIZE, ignitionTable.xSize); //Write the ignition Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG3_YSIZE, ignitionTable.ySize); //Write the ignition Table MAP/TPS dimension size

  for(int x=EEPROM_CONFIG3_MAP; x<EEPROM_CONFIG3_XBINS; x++)
  {
    offset = x - EEPROM_CONFIG3_MAP;
    EEPROM_update(x, ignitionTable.values[15-offset/16][offset%16]); //Write the 16x16 map with translation
  }
  //RPM bins
  for(int x=EEPROM_CONFIG3_XBINS; x<EEPROM_CONFIG3_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG3_XBINS;
    EEPROM_update(x, byte(ignitionTable.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG3_YBINS; x<EEPROM_CONFIG4_START; x++)
  {
    offset = x - EEPROM_CONFIG3_YBINS;
    EEPROM_update(x, ignitionTable.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
  }
  //That concludes the writing of the IGN table
  //*********************************************************************************************************************************************************************************
  ...
  for(int x=EEPROM_CONFIG4_START; x<EEPROM_CONFIG4_END; x++)
  {
    EEPROM_update(x, *(pnt_configPage + byte(x - EEPROM_CONFIG4_START)));
  }
  //*********************************************************************************************************************************************************************************
  ...
  -----------------------------------------------------*/
  //Begin writing the Ignition table, basically the same thing as above
  EEPROM_update(EEPROM_CONFIG5_XSIZE, afrTable.xSize); //Write the ignition Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG5_YSIZE, afrTable.ySize); //Write the ignition Table MAP/TPS dimension size

  for(int x=EEPROM_CONFIG5_MAP; x<EEPROM_CONFIG5_XBINS; x++)
  {
    offset = x - EEPROM_CONFIG5_MAP;
    EEPROM_update(x, afrTable.values[15-offset/16][offset%16]); //Write the 16x16 map
  }
  //RPM bins
  for(int x=EEPROM_CONFIG5_XBINS; x<EEPROM_CONFIG5_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG5_XBINS;
    EEPROM_update(x, byte(afrTable.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG5_YBINS; x<EEPROM_CONFIG6_START; x++)
  {
    offset = x - EEPROM_CONFIG5_YBINS;
    EEPROM_update(x, afrTable.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
  }
  //That concludes the writing of the AFR table
  //*********************************************************************************************************************************************************************************
  ...
  for(int x=EEPROM_CONFIG6_START; x<EEPROM_CONFIG6_END; x++)
  {
    EEPROM_update(x, *(pnt_configPage + byte(x - EEPROM_CONFIG6_START)));
  }
  //*********************************************************************************************************************************************************************************
  ...
  for(int x=EEPROM_CONFIG7_START; x<EEPROM_CONFIG7_END; x++)
  {
    EEPROM_update(x, *(pnt_configPage + byte(x - EEPROM_CONFIG7_START)));
  }
  ...
  -----------------------------------------------------*/
  //Begin writing the 2 tables, basically the same thing as above but we're doing these 2 together (2 tables per page instead of 1)
  EEPROM_update(EEPROM_CONFIG8_XSIZE1, boostTable.xSize); //Write the boost Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG8_YSIZE1, boostTable.ySize); //Write the boost Table MAP/TPS dimension size
  EEPROM_update(EEPROM_CONFIG8_XSIZE2, vvtTable.xSize); //Write the vvt Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG8_YSIZE2, vvtTable.ySize); //Write the vvt Table MAP/TPS dimension size

  int y = EEPROM_CONFIG8_MAP2; //We do the 2 maps together in the same loop
  for(int x=EEPROM_CONFIG8_MAP1; x<EEPROM_CONFIG8_XBINS1; x++)
  {
    offset = x - EEPROM_CONFIG8_MAP1;
    EEPROM_update(x, boostTable.values[7-offset/8][offset%8]); //Write the 8x8 map
    offset = y - EEPROM_CONFIG8_MAP2;
    EEPROM_update(y, vvtTable.values[7-offset/8][offset%8]); //Write the 8x8 map
    y++;
  }
  //RPM bins
  y = EEPROM_CONFIG8_XBINS2;
  ...
  {
    offset = x - EEPROM_CONFIG8_XBINS1;
    EEPROM_update(x, byte(boostTable.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    offset = y - EEPROM_CONFIG8_XBINS2;
    EEPROM_update(x, byte(vvtTable.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    y++;
  }
  //TPS/MAP bins
  y=EEPROM_CONFIG8_YBINS2;
  ...
  {
    offset = x - EEPROM_CONFIG8_YBINS1;
    EEPROM_update(x, boostTable.axisY[offset]); //TABLE_LOAD_MULTIPLIER is NOT used for boost as it is TPS based (0-100)
    offset = y - EEPROM_CONFIG8_YBINS2;
    EEPROM_update(y, vvtTable.axisY[offset]); //TABLE_LOAD_MULTIPLIER is NOT used for VVT as it is TPS based (0-100)
    y++;
  }
  ...
  -----------------------------------------------------*/
  //Begin writing the 2 tables, basically the same thing as above but we're doing these 2 together (2 tables per page instead of 1)
  EEPROM_update(EEPROM_CONFIG9_XSIZE1,trim1Table.xSize); //Write the boost Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG9_YSIZE1,trim1Table.ySize); //Write the boost Table MAP/TPS dimension size
  EEPROM_update(EEPROM_CONFIG9_XSIZE2,trim2Table.xSize); //Write the boost Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG9_YSIZE2,trim2Table.ySize); //Write the boost Table MAP/TPS dimension size
  EEPROM_update(EEPROM_CONFIG9_XSIZE3,trim3Table.xSize); //Write the boost Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG9_YSIZE3,trim3Table.ySize); //Write the boost Table MAP/TPS dimension size
  EEPROM_update(EEPROM_CONFIG9_XSIZE4,trim4Table.xSize); //Write the boost Table RPM dimension size
  EEPROM_update(EEPROM_CONFIG9_YSIZE4,trim4Table.ySize); //Write the boost Table MAP/TPS dimension size
  ...
  {
    offset = x - EEPROM_CONFIG9_MAP1;
    EEPROM_update(x, trim1Table.values[5-offset/6][offset%6]);  //Write the 6x6 map
    offset = y - EEPROM_CONFIG9_MAP2;
    EEPROM_update(y, trim2Table.values[5-offset/6][offset%6]);  //Write the 6x6 map
    offset = z - EEPROM_CONFIG9_MAP3;
    EEPROM_update(z, trim3Table.values[5-offset/6][offset%6]);  //Write the 6x6 map
    offset = i - EEPROM_CONFIG9_MAP4;
    EEPROM_update(i, trim4Table.values[5-offset/6][offset%6]);  //Write the 6x6 map
    y++;
    z++;
    i++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG9_XBINS1;
    EEPROM_update(x, byte(trim1Table.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    offset = y - EEPROM_CONFIG9_XBINS2;
    EEPROM_update(y, byte(trim2Table.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    offset = z - EEPROM_CONFIG9_XBINS3;
    EEPROM_update(z, byte(trim3Table.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    offset = i - EEPROM_CONFIG9_XBINS4;
    EEPROM_update(i, byte(trim4Table.axisX[offset]/TABLE_RPM_MULTIPLIER)); //RPM bins are divided by 100 and converted to a byte
    y++;
    z++;
    i++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG9_YBINS1;
    EEPROM_update(x, trim1Table.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
    offset = y - EEPROM_CONFIG9_YBINS2;
    EEPROM_update(y, trim2Table.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
    offset = z - EEPROM_CONFIG9_YBINS3;
    EEPROM_update(z, trim3Table.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
    offset = i - EEPROM_CONFIG9_YBINS4;
    EEPROM_update(i, trim4Table.axisY[offset]/TABLE_LOAD_MULTIPLIER); //Table load is divided by 2 (Allows for MAP up to 511)
    y++;
    z++;
    i++;
  }
  ...
  for(int x=EEPROM_CONFIG10_START; x<EEPROM_CONFIG10_END; x++)
  {
    EEPROM_update(x, *(pnt_configPage + byte(x - EEPROM_CONFIG10_START)));
  }
  //*********************************************************************************************************************************************************************************
...
void loadConfig_STM()
{
  ...
  {
    offset = x - EEPROM_CONFIG1_MAP;
    fuelTable.values[15-offset/16][offset%16] = EEPROM_read(x); //Read the 8x8 map
  }
  //RPM bins
  for(int x=EEPROM_CONFIG1_XBINS; x<EEPROM_CONFIG1_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG1_XBINS;
    fuelTable.axisX[offset] = (EEPROM_read(x) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG1_YBINS; x<EEPROM_CONFIG2_START; x++)
  {
    offset = x - EEPROM_CONFIG1_YBINS;
    fuelTable.axisY[offset] = EEPROM_read(x) * TABLE_LOAD_MULTIPLIER;
  }

  pnt_configPage = (byte *)&configPage1; //Create a pointer to Page 1 in memory
  for(int x=EEPROM_CONFIG2_START; x<EEPROM_CONFIG2_END; x++)
  {
    *(pnt_configPage + byte(x - EEPROM_CONFIG2_START)) = EEPROM_read(x);
  }
  //That concludes the reading of the VE table
  ...
  //Begin writing the Ignition table, basically the same thing as above
  //ignitionTable.xSize = EEPROM_read(EEPROM_CONFIG2_XSIZE); //Read the ignition Table RPM dimension size (Currently not supproted)
  //ignitionTable.ySize = EEPROM_read(EEPROM_CONFIG2_YSIZE); //Read the ignition Table MAP/TPS dimension size (Currently not supproted)

  for(int x=EEPROM_CONFIG3_MAP; x<EEPROM_CONFIG3_XBINS; x++)
  {
    offset = x - EEPROM_CONFIG3_MAP;
    ignitionTable.values[15-offset/16][offset%16] = EEPROM_read(x); //Read the 8x8 map
  }
  //RPM bins
  for(int x=EEPROM_CONFIG3_XBINS; x<EEPROM_CONFIG3_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG3_XBINS;
    ignitionTable.axisX[offset] = (EEPROM_read(x) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG3_YBINS; x<EEPROM_CONFIG4_START; x++)
  {
    offset = x - EEPROM_CONFIG3_YBINS;
    ignitionTable.axisY[offset] = EEPROM_read(x) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
  }

  pnt_configPage = (byte *)&configPage2; //Create a pointer to Page 2 in memory
  for(int x=EEPROM_CONFIG4_START; x<EEPROM_CONFIG4_END; x++)
  {
    *(pnt_configPage + byte(x - EEPROM_CONFIG4_START)) = EEPROM_read(x);
  }
  ...
  {
    offset = x - EEPROM_CONFIG5_MAP;
    afrTable.values[15-offset/16][offset%16] = EEPROM_read(x); //Read the 16x16 map
  }
  //RPM bins
  for(int x=EEPROM_CONFIG5_XBINS; x<EEPROM_CONFIG5_YBINS; x++)
  {
    offset = x - EEPROM_CONFIG5_XBINS;
    afrTable.axisX[offset] = (EEPROM_read(x) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
  }
  //TPS/MAP bins
  for(int x=EEPROM_CONFIG5_YBINS; x<EEPROM_CONFIG6_START; x++)
  {
    offset = x - EEPROM_CONFIG5_YBINS;
    afrTable.axisY[offset] = EEPROM_read(x) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
  }

  pnt_configPage = (byte *)&configPage3; //Create a pointer to Page 2 in memory
  for(int x=EEPROM_CONFIG6_START; x<EEPROM_CONFIG6_END; x++)
  {
    *(pnt_configPage + byte(x - EEPROM_CONFIG6_START)) = EEPROM_read(x);
  }
  ...
  for(int x=EEPROM_CONFIG7_START; x<EEPROM_CONFIG7_END; x++)
  {
    *(pnt_configPage + byte(x - EEPROM_CONFIG7_START)) = EEPROM_read(x);
  }
  ...
  {
    offset = x - EEPROM_CONFIG8_MAP1;
    boostTable.values[7-offset/8][offset%8] = EEPROM_read(x); //Read the 8x8 map
    offset = y - EEPROM_CONFIG8_MAP2;
    vvtTable.values[7-offset/8][offset%8] = EEPROM_read(y); //Read the 8x8 map
    y++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG8_XBINS1;
    boostTable.axisX[offset] = (EEPROM_read(x) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    offset = y - EEPROM_CONFIG8_XBINS2;
    vvtTable.axisX[offset] = (EEPROM_read(y) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    y++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG8_YBINS1;
    boostTable.axisY[offset] = EEPROM_read(x); //TABLE_LOAD_MULTIPLIER is NOT used for boost as it is TPS based (0-100)
    offset = y - EEPROM_CONFIG8_YBINS2;
    vvtTable.axisY[offset] = EEPROM_read(y); //TABLE_LOAD_MULTIPLIER is NOT used for VVT as it is TPS based (0-100)
    y++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG9_MAP1;
    trim1Table.values[5-offset/6][offset%6] = EEPROM_read(x); //Read the 6x6 map
    offset = y - EEPROM_CONFIG9_MAP2;
    trim2Table.values[5-offset/6][offset%6] = EEPROM_read(y); //Read the 6x6 map
    offset = z - EEPROM_CONFIG9_MAP3;
    trim3Table.values[5-offset/6][offset%6] = EEPROM_read(z); //Read the 6x6 map
    offset = i - EEPROM_CONFIG9_MAP4;
    trim4Table.values[5-offset/6][offset%6] = EEPROM_read(i); //Read the 6x6 map
    y++;
    z++;
    i++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG9_XBINS1;
    trim1Table.axisX[offset] = (EEPROM_read(x) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    offset = y - EEPROM_CONFIG9_XBINS2;
    trim2Table.axisX[offset] = (EEPROM_read(y) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    offset = z - EEPROM_CONFIG9_XBINS3;
    trim3Table.axisX[offset] = (EEPROM_read(z) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    offset = i - EEPROM_CONFIG9_XBINS4;
    trim4Table.axisX[offset] = (EEPROM_read(i) * TABLE_RPM_MULTIPLIER); //RPM bins are divided by 100 when stored. Multiply them back now
    y++;
    z++;
    i++;
  }
  ...
  {
    offset = x - EEPROM_CONFIG9_YBINS1;
    trim1Table.axisY[offset] = EEPROM_read(x) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
    offset = y - EEPROM_CONFIG9_YBINS2;
    trim2Table.axisY[offset] = EEPROM_read(y) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
    offset = z - EEPROM_CONFIG9_YBINS3;
    trim3Table.axisY[offset] = EEPROM_read(z) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
    offset = i - EEPROM_CONFIG9_YBINS4;
    trim4Table.axisY[offset] = EEPROM_read(i) * TABLE_LOAD_MULTIPLIER; //Table load is divided by 2 (Allows for MAP up to 511)
    y++;
    z++;
    i++;
  }
  ...
  for(int x=EEPROM_CONFIG10_START; x<EEPROM_CONFIG10_END; x++)
  {
    *(pnt_configPage + byte(x - EEPROM_CONFIG10_START)) = EEPROM_read(x);
  }
