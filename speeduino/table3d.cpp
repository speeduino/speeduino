...
{
  #define CTA_GET_ROW_ITERATOR(size, pTable) \
      return ((TABLE3D_TYPENAME_BASE(size)*)pTable)->values.begin();
...
{
  #define CTA_GET_X_ITERATOR(size, pTable) \
      return ((TABLE3D_TYPENAME_BASE(size)*)pTable)->axisX.begin();
...
{
  #define CTA_GET_X_RITERATOR(size, pTable) \
      return ((TABLE3D_TYPENAME_BASE(size)*)pTable)->axisX.rbegin();
  #define CTA_GET_X_ITERATOR_DEFAULT ({ return table_axis_iterator(NULL, NULL); })      
...
{
  #define CTA_GET_Y_ITERATOR(size, pTable) \
      return ((TABLE3D_TYPENAME_BASE(size)*)pTable)->axisY.begin();
...
{
  #define CTA_GET_Y_RITERATOR(size, pTable) \
      return ((TABLE3D_TYPENAME_BASE(size)*)pTable)->axisY.rbegin();
  #define CTA_GET_Y_ITERATOR_DEFAULT ({ return table_axis_iterator(NULL, NULL); })      
