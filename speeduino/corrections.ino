...
static inline byte correctionsFuel_new(void)
{
  ...
  sumCorrections =
      currentStatus.wueCorrection + currentStatus.ASEValue + correctionCrankingValue + currentStatus.AEamount + correctionFloodClearValue + currentStatus.batCorrection + currentStatus.iatCorrection + currentStatus.baroCorrection + currentStatus.flexCorrection + currentStatus.launchCorrection;
...
byte correctionFlex(void)
{
  ...
  if(configPage2.flexEnabled == 1) { flexValue = table2D_getValue(&flexFuelTable, currentStatus.ethanolPct); }
  return flexValue;
}
...
byte correctionFuelTemp(void)
{
  ...
  if(configPage2.flexEnabled == 1) { fuelTempValue = table2D_getValue(&fuelTempTable, currentStatus.fuelTemp + CALIBRATION_TEMPERATURE_OFFSET); }
...
byte correctionAFRClosedLoop(void)
{
  ...
  if((configPage6.egoType > 0) && (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) != 1)) // egoType of 0 means no O2 sensor. If DFCO is active do not run the ego controllers to prevent interator wind-up.
  {
    ...
    if((ignitionCount >= AFRnextCycle) || (ignitionCount < (AFRnextCycle - configPage6.egoCount)))
    {
      ...
      if((currentStatus.coolant > (int)(configPage6.egoTemp - CALIBRATION_TEMPERATURE_OFFSET)) && (currentStatus.RPM > (unsigned int)(configPage6.egoRPM * 100)) && (currentStatus.TPS <= configPage6.egoTPSMax) && (currentStatus.O2 < configPage6.ego_max) && (currentStatus.O2 > configPage6.ego_min) &&
         (currentStatus.runSecs > configPage6.ego_sdelay) && (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) == 0))
      {
...
int8_t correctionIdleAdvance(int8_t advance)
{
  ...
  */
  ...
  if((!idleAdvActive && BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)) && ((configPage6.iacAlgorithm == 0) || (currentStatus.RPM > (((uint16_t)currentStatus.CLIdleTarget * 10) - (uint16_t)IGN_IDLE_THRESHOLD)))) { idleAdvActive = true; }
...
int8_t correctionNitrous(int8_t advance)
{
  ...
  if(configPage10.n2o_enable > 0)
  {
    ...
    if((currentStatus.nitrous_status == NITROUS_STAGE1) || (currentStatus.nitrous_status == NITROUS_BOTH)) { ignNitrous -= configPage10.n2o_stage1_retard; }
    if((currentStatus.nitrous_status == NITROUS_STAGE2) || (currentStatus.nitrous_status == NITROUS_BOTH)) { ignNitrous -= configPage10.n2o_stage2_retard; }
  }
