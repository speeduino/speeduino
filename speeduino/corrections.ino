...
int8_t correctionIdleAdvance(int8_t advance)
{
  ...
  if( (configPage2.idleAdvEnabled >= 1) && (currentStatus.runSecs >= configPage2.IdleAdvDelay))
  {
    ...
    if( (configPage2.idleAdvAlgorithm == 0) && ((currentStatus.RPM < (unsigned int)(configPage2.idleAdvRPM * 100)) && (currentStatus.TPS < configPage2.idleAdvTPS)) && (( currentStatus.vss < configPage2.idleAdvVss) || configPage2.vssMode == 0 )) // TPS based idle state
    {
    ...
    else if( (configPage2.idleAdvAlgorithm == 1) && (currentStatus.RPM < (unsigned int)(configPage2.idleAdvRPM * 100) && (currentStatus.CTPSActive == 1) ) && (( currentStatus.vss < configPage2.idleAdvVss) || configPage2.vssMode == 0 )) // closed throttle position sensor (CTPS) based idle state
    {
      int8_t advanceIdleAdjust = (int16_t)(table2D_getValue(&idleAdvanceTable, idleRPMdelta)) - 15;
      if(configPage2.idleAdvEnabled == 1) { ignIdleValue = (advance + advanceIdleAdjust); }
      else if(configPage2.idleAdvEnabled == 2) { ignIdleValue = advanceIdleAdjust; }
