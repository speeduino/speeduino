...
void receiveCANwbo() // rusEFI CAN Wideband support https://github.com/mck1117/wideband
{
  if (configPage2.canREWBO == 1)
    {
      outMsg.id = 0xEF50000;
      ...
      Can0.write(outMsg);
    if ((inMsg.id == 0x190 || inMsg.id == 0x192) && inMsg.buf[1] == 1)
    {
      uint32_t inLambda;
      ...
      {
        case 0x190:
        currentStatus.O2 = temp_AFR >= 255 ? 255: temp_AFR; // If temp_AFR is greater than 255 we write 255 to currentStatus.O2 since its an 8 bit integer hence the max for it is 255 otherwise we write the other value
        break;

        case 0x192:
        currentStatus.O2_2 = temp_AFR >= 255 ? 255: temp_AFR; // If temp_AFR is greater than 255 we write 255 to currentStatus.O2 since its an 8 bit integer hence the max for it is 255 otherwise we write the other value
        break;
