...
void reciveCANwbo() // RusEFI CAN Wideband support https://github.com/mck1117/wideband
  {
    if(configPage15.canREWBOCAN == true) // Enable heater once engine reaches 40C to avoid heater running on cold engine with ignition on
    {
      ...
      outMsg.buf[1] = currentStatus.coolant <= 40? 0x00 : 0x01;
      Can0.write(outMsg);
    }
    if (inMsg.id == 400 && configPage15.canREWBOCAN == true)
    {
      uint16_t inLambda;
      inLambda = (word(inMsg.buf[3], inMsg.buf[2])); // Combining 2 bytes of data into single variable and multiplying it 10000 since factor is 0.0001 and to avoid floating point
      if((inLambda > 5 || inLambda < 14) && inMsg.buf[1] == 1) // Checking if lambda is within usable range of 0.5 and 1.4 and if valid bit is present from the controller
      {
        currentStatus.O2 = (unsigned int)(inLambda * configPage2.stoich / 10000); // Multiplying lambda by stoich ratio to get AFR and dividing it by 10 to get correct value
      }
    }
  }
