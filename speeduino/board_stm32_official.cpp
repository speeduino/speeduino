...
STM32RTC& rtc = STM32RTC::getInstance();
  ...
  void initBoard()
  {
    ...
    */
    ...
    {
        idle_pwm_max_count = MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.idleFreq * 2U); //Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle. Note that the frequency is divided by 2 coming from TS to allow for up to 5KHz
    } 
    ...
    */
    ...
    boost_pwm_max_count = MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.boostFreq * 2U); //Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle. The x2 is there because the frequency is stored at half value (in a byte) to allow frequencies up to 511Hz
    vvt_pwm_max_count = MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.vvtFreq * 2U); //Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle
    fan_pwm_max_count = MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.fanFreq * 2U); //Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle
