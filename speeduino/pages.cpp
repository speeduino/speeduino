...
public:
  ...
  { 
    switch (get_table_location())
    {
      ...
      case table_location_xaxis:
        return get_axis_io_converter(table_t::xaxis_t::domain).to_byte(get_xaxis_value());
      case table_location_yaxis:
      default:
        return get_axis_io_converter(table_t::yaxis_t::domain).to_byte(get_yaxis_value());
    }
  }
  ...
  inline offset_to_table &operator=( byte new_value )
  {
    switch (get_table_location())
    {
      ...
      case table_location_xaxis:
        get_xaxis_value() = get_axis_io_converter(table_t::xaxis_t::domain).from_byte(new_value);
        break;

      case table_location_yaxis:
      default:
        get_yaxis_value() = get_axis_io_converter(table_t::yaxis_t::domain).from_byte(new_value);
    }
