...
static inline write_location write(table_axis_iterator it, write_location location)
{
  while (location.maxWrites>0U && !it.at_end())
  {
    if (update(getStorageAPI(), location.address, *it)) {
      --location.maxWrites;
    }
    ++location.address;
    ++it;
...
void loadAllCalibrationTables(void)
{
  ...
  loadObject(getStorageAPI(), EEPROM_CALIBRATION_O2_BINS, o2CalibrationTable.axis);
  loadObject(getStorageAPI(), EEPROM_CALIBRATION_O2_VALUES, o2CalibrationTable.values);
  
  loadObject(getStorageAPI(), EEPROM_CALIBRATION_IAT_BINS, iatCalibrationTable.axis);
  loadObject(getStorageAPI(), EEPROM_CALIBRATION_IAT_VALUES, iatCalibrationTable.values);

  loadObject(getStorageAPI(), EEPROM_CALIBRATION_CLT_BINS, cltCalibrationTable.axis);
  loadObject(getStorageAPI(), EEPROM_CALIBRATION_CLT_VALUES, cltCalibrationTable.values);
}
