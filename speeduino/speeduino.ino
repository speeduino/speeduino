...
void loop()
{
  ...
  {
    if (Serial.availableForWrite() > 16) {
      sendToothLog(inProgressOffset);
    }
  }
  if ( compositeLogSendInProgress == true)
  {
    if (Serial.availableForWrite() > 16) {
      sendCompositeLog(inProgressOffset);
    }
  }
  ...
  else
  {
    ...
    if ( (currentStatus.toothLogEnabled == false) && (currentStatus.compositeLogEnabled == false) ) {
      initialiseTriggers();
    }
    ...
    if (configPage4.ignBypassEnabled > 0) {
      digitalWrite(pinIgnBypass, LOW);  //Reset the ignition bypass ready for next crank attempt
    }
  }
  ...
  if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_15HZ)) //Every 32 loops
  {
    ...
    if (configPage6.flatSEnable || configPage6.launchEnabled) {
      if (configPage6.launchHiLo > 0) {
        clutchTrigger = digitalRead(pinLaunch);
      }
      else {
        clutchTrigger = !digitalRead(pinLaunch);
      }
    ...
    if (previousClutchTrigger != clutchTrigger) {
    ...
    else
    {
      ...
      if (configPage6.flatSEnable && clutchTrigger && (currentStatus.RPM > ((unsigned int)(configPage6.flatSArm) * 100)) && (currentStatus.RPM > currentStatus.clutchEngagedRPM) ) {
  ...
  if (currentStatus.hasSync && (currentStatus.RPM > 0))
  {
    if (currentStatus.startRevolutions >= configPage4.StgCycles)  {
      ignitionOn = true;  //Enable the fuel and ignition, assuming staging revolutions are complete
      fuelOn = true;
    }
    ...
    if (configPage2.strokes == FOUR_STROKE) {
      pwLimit = pwLimit * 2 / currentStatus.nSquirts;
    }
    else {
    ...
    if ( (configPage10.stagingEnabled == true) && (configPage2.nCylinders <= INJ_CHANNELS || configPage2.injType == INJ_TYPE_TBODY) && (currentStatus.PW1 > inj_opentime_uS) ) //Final check is to ensure that DFCO isn't active, which would cause an overflow below (See #267)
    {
      ...
      else if (configPage10.stagingMode == STAGING_MODE_AUTO)
      {
        ...
        if (tempPW1 > pwLimit)
        {
          ...
          currentStatus.PW3 = 0;  //If tempPW1 < pwLImit it means that the entire fuel load can be handled by the primaries. Simply set the secondaries to 0
        }
      }
      ...
      case 4:
        ...
        if (configPage2.injLayout == INJ_SEQUENTIAL)
        {
          ...
          if (configPage6.fuelTrimEnabled > 0)
          {
            ...
            if (pw1percent != 100) {
              currentStatus.PW1 = (pw1percent * currentStatus.PW1) / 100;
            }
            if (pw2percent != 100) {
              currentStatus.PW2 = (pw2percent * currentStatus.PW2) / 100;
            }
            if (pw3percent != 100) {
              currentStatus.PW3 = (pw3percent * currentStatus.PW3) / 100;
            }
            if (pw4percent != 100) {
              currentStatus.PW4 = (pw4percent * currentStatus.PW4) / 100;
            }
          }
        }
      ...
      case 6:
        ...
        if (configPage2.injLayout == INJ_SEQUENTIAL)
        {
          ...
          if (configPage6.fuelTrimEnabled > 0)
          {
            ...
            if (pw5percent != 100) {
              currentStatus.PW5 = (pw5percent * currentStatus.PW5) / 100;
            }
            if (pw6percent != 100) {
              currentStatus.PW6 = (pw6percent * currentStatus.PW6) / 100;
            }
          }
        }
#endif
        ...
        */
        ...
        if (configPage2.injLayout == INJ_SEQUENTIAL)
        {
          ...
          if (configPage6.fuelTrimEnabled > 0)
          {
            ...
            if (pw7percent != 100) {
              currentStatus.PW7 = (pw7percent * currentStatus.PW7) / 100;
            }
            if (pw8percent != 100) {
              currentStatus.PW8 = (pw8percent * currentStatus.PW8) / 100;
            }
          }
        }
#endif
    ...
    while (crankAngle > CRANK_ANGLE_MAX_INJ ) {
      crankAngle = crankAngle - CRANK_ANGLE_MAX_INJ;  //Continue reducing the crank angle by the max injection amount until it's below the required limit. This will usually only run (at most) once, but in cases where there is sequential ignition and more than 2 squirts per cycle, it may run up to 4 times.
    }
      ...
      if ( (currentStatus.RPMdiv100 > configPage4.engineProtectMaxRPM) || currentStatus.launchingHard || currentStatus.flatShiftingHard) //Ugly, but the launch/flat shift check needs to be here as well to prevent these limiters not happening when under the the Engine Protection min rpm
      {
        ...
        else
        {
          ...
          if (currentStatus.startRevolutions > (rollingCutLastRev + 1) )
          {
            ...
            curRollingCut = 0;
            rollingCutCounter += 1;
            if (rollingCutCounter > (maxIgnOutputs - 1)) {
              rollingCutCounter = 0;
            }
        ...
        currentStatus.engineProtectStatus = 0;  //Force all engine protection flags to be off as we're below the minimum RPM
      }
    } //Protection active check
    else {
      curRollingCut = 0;  //Disables the rolling hard cut
    }
    ...
    if (fuelOn && !BIT_CHECK(currentStatus.status1, BIT_STATUS1_BOOSTCUT))
    {
      ...
      {
        if ( (injector1StartAngle <= crankAngle) && (fuelSchedule1.Status == RUNNING) ) {
          injector1StartAngle += CRANK_ANGLE_MAX_INJ;
        }
      ...
      */
      ...
      if ( (channel2InjEnabled) && (currentStatus.PW2 >= inj_opentime_uS) )
      {
        ...
        if ( tempCrankAngle < 0) {
          tempCrankAngle += CRANK_ANGLE_MAX_INJ;
        }
        tempStartAngle = injector2StartAngle - channel2InjDegrees;
        if ( tempStartAngle < 0) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule2.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
      ...
      if ( (channel3InjEnabled) && (currentStatus.PW3 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule3.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
      ...
      if ( (channel4InjEnabled) && (currentStatus.PW4 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule4.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
      ...
      if ( (channel5InjEnabled) && (currentStatus.PW5 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule5.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
      ...
      if ( (channel6InjEnabled) && (currentStatus.PW6 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule6.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
      ...
      if ( (channel7InjEnabled) && (currentStatus.PW7 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule7.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
      ...
      if ( (channel8InjEnabled) && (currentStatus.PW8 >= inj_opentime_uS) )
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (fuelSchedule8.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_INJ;
        }
        if ( tempStartAngle > tempCrankAngle )
        {
    ...
    if (ignitionOn)
    {
...
#if IGN_CHANNELS >= 1
      if ( (ignition1StartAngle <= crankAngle) && (ignitionSchedule1.Status == RUNNING) ) {
        ignition1StartAngle += CRANK_ANGLE_MAX_IGN;
      }
      ...
      if ( (ignitionSchedule1.Status == RUNNING) && (ignition1EndAngle > crankAngle) && (configPage4.StgCycles == 0) && (configPage2.perToothIgn != true) )
      {
        ...
        if (ignition1EndAngle > crankAngle) {
          ...
          uSToEnd = fastDegreesToUS( (360 + ignition1EndAngle - crankAngle) );
        }
        //*********
      ...
      if (maxIgnOutputs >= 2)
      {
        ...
        if ( tempCrankAngle < 0) {
          tempCrankAngle += CRANK_ANGLE_MAX_IGN;
        }
        tempStartAngle = ignition2StartAngle - channel2IgnDegrees;
        if ( tempStartAngle < 0) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }

        unsigned long ignition2StartTime = 0;
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule2.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition2StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition2StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition2StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 3)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule3.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition3StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition3StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition3StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 4)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule4.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition4StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition4StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition4StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 5)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule5.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition5StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition5StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition5StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 6)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule6.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition6StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition6StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition6StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 7)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule7.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition7StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition7StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition7StartTime = 0;
        }
      ...
      if (maxIgnOutputs >= 8)
      {
        ...
        if ( (tempStartAngle <= tempCrankAngle) && (ignitionSchedule8.Status == RUNNING) ) {
          tempStartAngle += CRANK_ANGLE_MAX_IGN;
        }
        if (tempStartAngle > tempCrankAngle) {
          ignition8StartTime = angleToTime((tempStartAngle - tempCrankAngle), CRANKMATH_METHOD_INTERVAL_REV);
        }
        //else if (tempStartAngle < tempCrankAngle) { ignition8StartTime = ((long)(360 - tempCrankAngle + tempStartAngle) * (long)timePerDegree); }
        else {
          ignition8StartTime = 0;
        }
...
byte getVE1()
{
  ...
  else if (configPage2.fuelAlgorithm == LOAD_SOURCE_IMAPEMAP)
  {
    ...
    currentStatus.fuelLoad = currentStatus.MAP;  //Fallback position
  }
...
uint16_t calculateInjectorStartAngle(uint16_t PWdivTimerPerDegree, int16_t injChannelDegrees)
{
  ...
  if (tempInjectorStartAngle < PWdivTimerPerDegree) {
    tempInjectorStartAngle += CRANK_ANGLE_MAX_INJ;
  }
  tempInjectorStartAngle -= PWdivTimerPerDegree;
  while (tempInjectorStartAngle > (uint16_t)CRANK_ANGLE_MAX_INJ) {
    tempInjectorStartAngle -= CRANK_ANGLE_MAX_INJ;
  }
...
void calculateIgnitionAngle2(int dwellAngle)
{
  ...
  if (ignition2EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition2EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition2StartAngle = ignition2EndAngle - dwellAngle;
  if (ignition2StartAngle < 0) {
    ignition2StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle3(int dwellAngle)
{
  ...
  if (ignition3EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition3EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition3StartAngle = ignition3EndAngle - dwellAngle;
  if (ignition3StartAngle < 0) {
    ignition3StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle3(int dwellAngle, int rotarySplitDegrees)
{
  ...
  ignition3StartAngle = ignition3EndAngle - dwellAngle;
  if (ignition3StartAngle > CRANK_ANGLE_MAX_IGN) {
    ignition3StartAngle -= CRANK_ANGLE_MAX_IGN;
  }
  if (ignition3StartAngle < 0) {
    ignition3StartAngle += CRANK_ANGLE_MAX_IGN;
...
void calculateIgnitionAngle4(int dwellAngle)
{
  ...
  if (ignition4EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition4EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition4StartAngle = ignition4EndAngle - dwellAngle;
  if (ignition4StartAngle < 0) {
    ignition4StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle4(int dwellAngle, int rotarySplitDegrees)
{
  ...
  ignition4StartAngle = ignition4EndAngle - dwellAngle;
  if (ignition4StartAngle > CRANK_ANGLE_MAX_IGN) {
    ignition4StartAngle -= CRANK_ANGLE_MAX_IGN;
  }
  if (ignition4StartAngle < 0) {
    ignition4StartAngle += CRANK_ANGLE_MAX_IGN;
...
void calculateIgnitionAngle5(int dwellAngle)
{
  ...
  if (ignition5EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition5EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition5StartAngle = ignition5EndAngle - dwellAngle;
  if (ignition5StartAngle < 0) {
    ignition5StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle6(int dwellAngle)
{
  ...
  if (ignition6EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition6EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition6StartAngle = ignition6EndAngle - dwellAngle;
  if (ignition6StartAngle < 0) {
    ignition6StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle7(int dwellAngle)
{
  ...
  if (ignition7EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition7EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition7StartAngle = ignition7EndAngle - dwellAngle;
  if (ignition7StartAngle < 0) {
    ignition7StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
...
void calculateIgnitionAngle8(int dwellAngle)
{
  ...
  if (ignition8EndAngle > CRANK_ANGLE_MAX_IGN) {
    ignition8EndAngle -= CRANK_ANGLE_MAX_IGN;
  }
  ignition8StartAngle = ignition8EndAngle - dwellAngle;
  if (ignition8StartAngle < 0) {
    ignition8StartAngle += CRANK_ANGLE_MAX_IGN;
  }
}
    ...
    case 4:
      ...
      else if (configPage4.sparkMode == IGN_MODE_ROTARY)
      {
        ...
        if (configPage2.fuelAlgorithm == LOAD_SOURCE_MAP) {
          splitDegrees = table2D_getValue(&rotarySplitTable, currentStatus.MAP / 2);
        }
        else {
          splitDegrees = table2D_getValue(&rotarySplitTable, currentStatus.TPS / 2);
        }
