...
uint16_t PW(int REQ_FUEL, byte VE, long MAP, int corrections, int injOpen)
{
  ...
  if (intermediate != 0)
  {
    ...
    if ((configPage2.smallPwEnabled) && ((unsigned long) intermediate < ((unsigned long) table2D_getAxisValue(&smallPwTable, 3) * 10UL))) {    
      ...
      if ((lowPwValue > 0) && (lowPwValue < ((unsigned long) table2D_getAxisValue(&smallPwTable, 3) * 10UL))) { //Sanity check
