...
uint16_t PW(const int& REQ_FUEL, const byte& VE, const long& MAP, const uint16_t& corrections, const int& injOpen)
{
  ...
  uint16_t iVE = ((unsigned int)VE << 7) / 100;
  ...
  if ( (configPage2.includeAFR == true) && (configPage6.egoType == EGO_TYPE_WIDE) && (currentStatus.runSecs > configPage6.ego_sdelay) ) {
    ...
    uint16_t iAFR = ((unsigned int)currentStatus.O2 << 7) / currentStatus.afrTarget;  //Include AFR (vs target) if enabled
    intermediate = (intermediate * (unsigned long)iAFR) >> 7;
  }

  if ( (configPage2.incorporateAFR == true) && (configPage2.includeAFR == false) ) {
    uint16_t iAFR = ((unsigned int)configPage2.stoich << 7) / currentStatus.afrTarget;  //Incorporate stoich vs target AFR, if enabled.
    intermediate = (intermediate * (unsigned long)iAFR) >> 7;
  }
  ...
  uint16_t iCorrections = (corrections << bitShift) / 100;
