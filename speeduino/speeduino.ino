...
void loop(void)
{
  ...
  if(serialTransmitInProgress()) { serialTransmit(); }

  // Check for any new or in-progress requests from serial.
  if(Serial.available() > 0 || serialRecieveInProgress()) { serialReceive(); }
  ...
  if((timeToLastTooth < MAX_STALL_TIME) ||
     (toothLastToothTime >
      currentLoopTime)) // Check how long ago the last tooth was seen compared to now. If it was more than half a second ago then the engine is probably stopped. toothLastToothTime can be greater than currentLoopTime if a pulse occurs between getting the latest time and doing the comparison
  {
  ...
  if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_4HZ))
  {
    ...
    if(auxIsEnabled == true)
    {
      ...
      for(byte AuxinChan = 0; AuxinChan < 16; AuxinChan++)
      {
        ...
        if(((configPage9.caninput_sel[currentStatus.current_caninchannel] & 12) == 4) && (((configPage9.enable_secondarySerial == 1) && ((configPage9.enable_intcan == 0) && (configPage9.intcan_available == 1))) ||
                                                                                          ((configPage9.enable_secondarySerial == 1) && ((configPage9.enable_intcan == 1) && (configPage9.intcan_available == 1)) && ((configPage9.caninput_sel[currentStatus.current_caninchannel] & 64) == 0)) ||
        ...
        else if(((configPage9.caninput_sel[currentStatus.current_caninchannel] & 12) == 4) && (((configPage9.enable_secondarySerial == 1) && ((configPage9.enable_intcan == 1) && (configPage9.intcan_available == 1)) && ((configPage9.caninput_sel[currentStatus.current_caninchannel] & 64) == 64)) ||
                                                                                               ((configPage9.enable_secondarySerial == 0) && ((configPage9.enable_intcan == 1) && (configPage9.intcan_available == 1)) && ((configPage9.caninput_sel[currentStatus.current_caninchannel] & 128) == 128))))
  ...
  if(BIT_CHECK(LOOP_TIMER, BIT_TIMER_1HZ)) // Once per second)
  {
    ...
    if((configPage10.wmiEnabled > 0) && (configPage10.wmiIndicatorEnabled > 0))
    {
      ...
      else { digitalWrite(pinWMIIndicator, configPage10.wmiIndicatorPolarity ? HIGH : LOW); }
    }
  ...
  if((configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_CL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL))
  {
  ...
  // Staging is possible by using the 6th channel if available
        ...
        break;
      // 6 cylinders
      case 6: injector2StartAngle = calculateInjectorStartAngle(PWdivTimerPerDegree, channel2InjDegrees, currentStatus.injAngle); injector3StartAngle = calculateInjectorStartAngle(PWdivTimerPerDegree, channel3InjDegrees, currentStatus.injAngle);
    ...
    } // Continue reducing the crank angle by the max injection amount until it's below the required limit. This will usually only run (at most) once, but in cases where there is sequential ignition and more than 2 squirts per cycle, it may run up to 4 times.
    ...
    uint16_t maxAllowedRPM = checkRevLimit(); // The maximum RPM allowed by all the potential limiters (Engine protection, 2-step, flat shift etc). Divided by 100. `checkRevLimit()` returns the current maximum RPM allow (divided by 100) based on either the fixed hard limit or the current coolant
                                              // temp
    ...
    } // Flat shifting is a special case as the RPM limit is based on when the clutch was engaged. It is not divided by 100 as it is set with the actual RPM
    ...
    if((configPage2.hardCutType == HARD_CUT_FULL) && (currentStatus.RPM > maxAllowedRPM))
    {
      ...
      switch(configPage6.engineProtectType)
      {
        case PROTECT_CUT_OFF:
          ...
          break;
        case PROTECT_CUT_IGN: ignitionChannelsOn = 0; break;
        case PROTECT_CUT_FUEL: fuelChannelsOn = 0; break;
    ...
    if((maxInjOutputs >= 1) && (currentStatus.PW1 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ1_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule1(timeOut, (unsigned long)currentStatus.PW1); }
    }
  #endif
    ...
    */
    ...
    if((maxInjOutputs >= 2) && (currentStatus.PW2 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ2_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule2(timeOut, (unsigned long)currentStatus.PW2); }
    }
  #endif
    ...
    if((maxInjOutputs >= 3) && (currentStatus.PW3 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ3_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule3(timeOut, (unsigned long)currentStatus.PW3); }
    }
  #endif
    ...
    if((maxInjOutputs >= 4) && (currentStatus.PW4 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ4_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule4(timeOut, (unsigned long)currentStatus.PW4); }
    }
  #endif
    ...
    if((maxInjOutputs >= 5) && (currentStatus.PW5 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ5_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule5(timeOut, (unsigned long)currentStatus.PW5); }
    }
  #endif
    ...
    if((maxInjOutputs >= 6) && (currentStatus.PW6 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ6_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule6(timeOut, (unsigned long)currentStatus.PW6); }
    }
  #endif
    ...
    if((maxInjOutputs >= 7) && (currentStatus.PW7 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ7_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule7(timeOut, (unsigned long)currentStatus.PW7); }
    }
  #endif
    ...
    if((maxInjOutputs >= 8) && (currentStatus.PW8 >= inj_opentime_uS) && (BIT_CHECK(fuelChannelsOn, INJ8_CMD_BIT)))
    {
      ...
      if(timeOut > 0U) { setFuelSchedule8(timeOut, (unsigned long)currentStatus.PW8); }
    }
  #endif
    ...
    if(ignitionChannelsOn > 0)
    {
      ...
      if(maxIgnOutputs >= 2)
      {
        ...
        if((ignition2StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN2_CMD_BIT))) { setIgnitionSchedule2(ign2StartFunction, ignition2StartTime, currentStatus.dwell + fixedCrankingOverride, ign2EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 3)
      {
        ...
        if((ignition3StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN3_CMD_BIT))) { setIgnitionSchedule3(ign3StartFunction, ignition3StartTime, currentStatus.dwell + fixedCrankingOverride, ign3EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 4)
      {
        ...
        if((ignition4StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN4_CMD_BIT))) { setIgnitionSchedule4(ign4StartFunction, ignition4StartTime, currentStatus.dwell + fixedCrankingOverride, ign4EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 5)
      {
        ...
        if((ignition5StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN5_CMD_BIT))) { setIgnitionSchedule5(ign5StartFunction, ignition5StartTime, currentStatus.dwell + fixedCrankingOverride, ign5EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 6)
      {
        ...
        if((ignition6StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN6_CMD_BIT))) { setIgnitionSchedule6(ign6StartFunction, ignition6StartTime, currentStatus.dwell + fixedCrankingOverride, ign6EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 7)
      {
        ...
        if((ignition7StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN7_CMD_BIT))) { setIgnitionSchedule7(ign7StartFunction, ignition7StartTime, currentStatus.dwell + fixedCrankingOverride, ign7EndFunction); }
      }
  #endif
      ...
      if(maxIgnOutputs >= 8)
      {
        ...
        if((ignition8StartTime > 0) && (BIT_CHECK(ignitionChannelsOn, IGN8_CMD_BIT))) { setIgnitionSchedule8(ign8StartFunction, ignition8StartTime, currentStatus.dwell + fixedCrankingOverride, ign8EndFunction); }
      }
  #endif
...
uint16_t PW(int REQ_FUEL, byte VE, long MAP, uint16_t corrections, int injOpen)
{
  ...
  if((configPage2.incorporateAFR == true) && (configPage2.includeAFR == false)) { intermediate = (intermediate * (unsigned long)iAFR) >> 7; }
  ...
  if(intermediate != 0)
  {
    ...
    {
      // AE Adds % of req_fuel
      if(configPage2.aeApplyMode == AE_MODE_ADDER) { intermediate += (((unsigned long)REQ_FUEL) * (currentStatus.AEamount - 100)) / 100; }
    }
...
void calculateIgnitionAngles(int dwellAngle)
{
  ...
  {
    // 1 cylinder
    case 1: calculateIgnitionAngle(dwellAngle, channel1IgnDegrees, currentStatus.advance, &ignition1EndAngle, &ignition1StartAngle); break;
    // 2 cylinders
    case 2:
    ...
    case 3:
      ...
      break;
    // 4 cylinders
    case 4: calculateIgnitionAngle(dwellAngle, channel1IgnDegrees, currentStatus.advance, &ignition1EndAngle, &ignition1StartAngle); calculateIgnitionAngle(dwellAngle, channel2IgnDegrees, currentStatus.advance, &ignition2EndAngle, &ignition2StartAngle);
