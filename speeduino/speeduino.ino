...
void setup()
{
  ...
  if ((currentStatus.MAP >= BARO_MIN) && (currentStatus.MAP <= BARO_MAX)) //Check if engine isn't running
  {
    ...
    #if defined(CORE_STM32)
      EEPROM.update(EEPROM_LAST_BARO>>1, currentStatus.baro);
    #else  
  ...
  else
  {
    ...
    #if defined(CORE_STM32)
      if ((EEPROM.read(EEPROM_LAST_BARO>>1) >= BARO_MIN) && (EEPROM.read(EEPROM_LAST_BARO>>1) <= BARO_MAX)) //Make sure it's not invalid (Possible on first run etc)
      { currentStatus.baro = EEPROM.read(EEPROM_LAST_BARO>>1); } //last baro correction
    #else  
    ...
    */
    ...
    if (currentStatus.hasSync && (currentStatus.RPM > 0))
    {
      ...
      */
      ...
      {
        rpmDelta = ((long)(micros() - toothOneTime) * (long)currentStatus.rpmDOT) / 1000000; //Take into account any likely accleration that has occurred since the last full revolution completed
      }
