...
TESTABLE_INLINE_STATIC void applyPwToInjectorChannels(const pulseWidths &pulse_widths, const config2 &page2, statuses &current) {
...
void __attribute__((always_inline)) loop(void)
{
    ...
    if ((currentStatus.hasSync || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)) && (currentStatus.RPM > 0))
    {
      ...
      calculateStaging(pulse_widths, configPage2, currentStatus);
