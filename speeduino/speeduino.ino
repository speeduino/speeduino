...
void loop()
{
    ...
    if (currentStatus.hasSync && (currentStatus.RPM > 0))
    {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // Check for quickshifter activated
      //Check for launching/flat shift (clutch) can be done around here too
      // digitalWrite(13,digitalRead(10));
      //Only check for pinLaunch if any function using it is enabled. Else pins might break starting a board
      if(configPage9.quickShiftEnable) {//||(configPage6.flatSEnable || configPage6.launchEnabled){
       previousClutchTrigger = clutchTrigger;
       if(configPage6.launchHiLo > 0) { clutchTrigger = digitalRead(pinLaunch); }
       else { clutchTrigger = !digitalRead(pinLaunch); }
       }
       // clutchTrigger = !digitalRead(11);
        if(previousClutchTrigger != clutchTrigger) { currentStatus.clutchEngagedTime = micros(); }
        
        //If quickShift enable
       if(configPage9.quickShiftEnable && clutchTrigger && (currentStatus.RPM > ((unsigned int)(configPage6.flatSArm) * 100)) && (micros()<(currentStatus.clutchEngagedTime + ((unsigned long)configPage9.quickShiftTime*1000)) ) ) { currentStatus.flatShiftingHard = true; }
        else { currentStatus.flatShiftingHard = false; }
  //      if(configPage9.quickShiftEnable && clutchTrigger  && (currentStatus.RPM > ((unsigned int)(configPage6.flatSArm) * 100)) && (micros()<currentStatus.clutchEngagedTime + 200000)) {currentStatus.flatShiftingHard = true;}
 //       else { currentStatus.flatShiftingHard = false; }
