...
static decoder_t buildDecoder(uint8_t decoder)
{
  ...
  static_assert(DECODER_MAX==_countof(initialisers), "Decoder initializer array mismatch");
...
void setDecoder(uint8_t decoderType)
{
  ...
  configPage2.perToothIgn = configPage2.perToothIgn && BIT_CHECK(decoderState, BIT_DECODER_PER_TOOTH_IGNITION);
}
