...
static inline map_adc_readings_t readMapSensors(const map_adc_readings_t &previousReadings, const config4 &page4, bool useEMAP) {
  return {
    readFilteredMapADC(pinNumbers.sensors.MAP, page4.ADCFILTER_MAP, previousReadings.mapADC),
    (uint16_t)(useEMAP ? readFilteredMapADC(pinNumbers.sensors.EMAP, page4.ADCFILTER_MAP, previousReadings.emapADC) : UINT16_MAX)
  };
}
...
static inline bool isCTPSSensorActive(void) {
  if(configPage2.CTPSPolarity == 0U) { return !digitalRead(pinNumbers.sensors.CTPS); } //Normal mode (ground switched)
  return digitalRead(pinNumbers.sensors.CTPS); //Inverted mode (5v activates closed throttle position sensor)
}
...
{
  readTPS((uint8_t)LOW_PASS_FILTER((uint16_t)fastMap10Bit(readAnalogSensor(pinNumbers.sensors.TPS), 0U, 255U), configPage4.ADCFILTER_TPS, (uint16_t)currentStatus.tpsADC));
}


void initialiseTPS(void) { 
  readTPS((uint8_t)fastMap10Bit(readAnalogSensor(pinNumbers.sensors.TPS), 0U, 255U)); // Need to read tps to detect flood clear state
}

static inline void readCLT(void)
{
  currentStatus.cltADC = LOW_PASS_FILTER(readAnalogSensor(pinNumbers.sensors.CLT), configPage4.ADCFILTER_CLT, currentStatus.cltADC);
...
void initialiseCLT(void) {
  currentStatus.cltADC = readAnalogSensor(pinNumbers.sensors.CLT);
  currentStatus.coolant = temperatureRemoveOffset(table2D_getValue(&cltCalibrationTable, currentStatus.cltADC)); //Temperature calibration values are stored as positive bytes. We subtract 40 from them to allow for negative temperatures
}

static inline void readIAT(void)
{
  currentStatus.iatADC = LOW_PASS_FILTER(readAnalogSensor(pinNumbers.sensors.IAT), configPage4.ADCFILTER_IAT, currentStatus.iatADC);
...
{
  uint16_t tempReading = mapADCToMAP(readMAPSensor(pinNumbers.sensors.MAP), configPage2.mapMin, configPage2.mapMax);
...
static inline void readBaro(void)
{
  ...
  {
    // readings
    setBaroFromSensorReading(LOW_PASS_FILTER(readMAPSensor(pinNumbers.sensors.baro), configPage4.ADCFILTER_BARO, currentStatus.baroADC)); //Very weak filter
...
void initialiseMAPBaro(void) 
{
  ...
  if ( configPage6.useExtBaro != 0U  )
  {
    ...
    setBaroFromSensorReading(readMAPSensor(pinNumbers.sensors.baro));
  }
  else
  {
...
static inline void readO2_1(void)
{
  ...
  {
    currentStatus.O2ADC = LOW_PASS_FILTER(readAnalogSensor(pinNumbers.sensors.O2), configPage4.ADCFILTER_O2, currentStatus.O2ADC);
...
static inline void readO2_2(void)
{
  ...
  if (pinNumbers.sensors.O2_2!=0U)
  {
    currentStatus.O2_2ADC = LOW_PASS_FILTER(readAnalogSensor(pinNumbers.sensors.O2_2), configPage4.ADCFILTER_O2, currentStatus.O2_2ADC);
...
{
  int16_t tempReading = fastMap10Bit(readAnalogSensor(pinNumbers.sensors.Bat), 0, 245); //Get the current raw Battery value. Permissible values are from 0v to 24.5v (245)
...
static inline byte getFuelPressure(void)
{
  ...
  {
    tempFuelPressure = fastMap10Bit(readAnalogSensor(pinNumbers.sensors.fuelPressure), configPage10.fuelPressureMin, configPage10.fuelPressureMax);
...
static inline byte getOilPressure(void)
{
  ...
  {
    //Perform ADC read
    tempOilPressure = fastMap10Bit(readAnalogSensor(pinNumbers.sensors.oilPressure), configPage10.oilPressureMin, configPage10.oilPressureMax);
...
void __attribute__((optimize("Os"))) initialiseFlexSensor(config2 &page2, statuses &current, uint8_t pin)
{
  ...
  page2.flexEnabled  = page2.flexEnabled && !pinIsOutput(pinNumbers.sensors.flex);
  if(page2.flexEnabled)
  {
    ...
    attachInterrupt(digitalPinToInterrupt(pinNumbers.sensors.flex), flexPulse, CHANGE); 
  }  
}
