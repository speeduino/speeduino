...
static void test_pinMapping(void)
{
  ...
  if (testPins.sensors.baro != NOT_A_PIN && testPins.sensors.baro == testPins.sensors.MAP)
  {
    TEST_MESSAGE("Baro pin is the same as MAP pin, which is allowed. Ignoring for uniqueness check.");
    testPins.sensors.baro = NOT_A_PIN; // Set baro pin to NOT_A_PIN for uniqueness check
  }
  // VSS pin is allowed to be the same as tertiary trigger, so we need to ignore that when checking for uniqueness
  if (testPins.pinVSS != NOT_A_PIN && testPins.pinVSS == testPins.triggerPins.tertiary)
  {
  ...
  if (testPins.pinBoost != NOT_A_PIN && testPins.pinBoost == testPins.idle.idle2)
  {
