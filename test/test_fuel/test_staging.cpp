...
extern void calculateStaging(uint16_t primaryPw, uint16_t pwLimit, uint16_t injOpenTime, const config2 &page2, const config10 &page10, statuses &current);
...
void test_Staging_Off(void)
{
  ...
  BIT_SET(current.status4, BIT_STATUS4_STAGING_ACTIVE);
  page10.stagingEnabled = false;

  calculateStaging(1000*4U, 9000, 1000, page2, page10, current);
  TEST_ASSERT_BIT_LOW(BIT_STATUS4_STAGING_ACTIVE, current.status4);
}
...
void test_Staging_4cyl_Auto_50pct(void)
{
  ...
  BIT_CLEAR(current.status4, BIT_STATUS4_STAGING_ACTIVE);
  page2.injLayout = INJ_PAIRED;
  page10.stagingEnabled = true;
  page10.stagingMode = STAGING_MODE_AUTO;
  ...
  // TEST_ASSERT_BIT_HIGH(BIT_STATUS4_STAGING_ACTIVE, current.status4);
}
