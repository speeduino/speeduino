...
extern uint16_t computePrimaryPulseWidth(uint16_t REQ_FUEL, uint16_t injOpenTime, const config2 &page2, const config6 &page6, const config10 &page10, const statuses &current);
...
static void test_computePrimaryPulseWidth_basic(void) {
  ...
  TEST_ASSERT_EQUAL(750+(750/4), computePrimaryPulseWidth(1000, 75, 1, 125, 0, context));
  // With -25% corrections (125)
  TEST_ASSERT_EQUAL(750-(750/4)-1, computePrimaryPulseWidth(1000, 75, 1,  75, 0, context));

  // With injection open time
  TEST_ASSERT_EQUAL(750+10, computePrimaryPulseWidth(1000, 75, 1, 100, 10, context));
}
...
static void test_computePrimaryPulseWidth_includeAFR(void) {
  // Positive test 
  auto context = getIncludeAFRContext();
  TEST_ASSERT_EQUAL(((750*3)/4), computePrimaryPulseWidth(1000, 75, 1, 100, 0, context));
...
static void test_computePrimaryPulseWidth_incorporateAFR(void) {
...
static void test_computePrimaryPulseWidth_MapMode100(void) {
  auto context = getBasicPwContext();

  context.page2.multiplyMAP = MULTIPLY_MAP_MODE_100;
  // Equal so no effect
  TEST_ASSERT_EQUAL(750,         computePrimaryPulseWidth(1000, 75, 100, 100, 0, context));
  // Map>100
  TEST_ASSERT_EQUAL(902 /*(750*MAP)/100*/, computePrimaryPulseWidth(1000, 75, 120, 100, 0, context));
  // Map<100
  TEST_ASSERT_EQUAL(597 /*(750*MAP)/100*/, computePrimaryPulseWidth(1000, 75, 80, 100, 0, context));
}

static void test_computePrimaryPulseWidth_MapModeBaro(void) {
  auto context = getBasicPwContext();

  context.page2.multiplyMAP = MULTIPLY_MAP_MODE_BARO;
  // Equal so no effect
  context.current.baro = 100;
  TEST_ASSERT_EQUAL(750, computePrimaryPulseWidth(1000, 75, context.current.baro,     100, 0, context));
  // MAP>baro
  context.current.baro = 90;
  TEST_ASSERT_EQUAL(996, computePrimaryPulseWidth(1000, 75, context.current.baro+30U, 100, 0, context));
  // MAP<baro
  context.current.baro = 120;
  TEST_ASSERT_EQUAL(562, computePrimaryPulseWidth(1000, 75, context.current.baro-30U, 100, 0, context));
}
...
static void test_computePrimaryPulseWidth_AeAdder(void) {
  // AE needs to be added
  auto context = getIncludeAeContext();
  TEST_ASSERT_EQUAL(800 /* (1000*0.75)+(1000*0.05) */, computePrimaryPulseWidth(1000, 75, 1, 100, 0, context));
...
static void test_computePrimaryPulseWidth_Nitrous(void) {
  ...
  TEST_ASSERT_GREATER_THAN(750, computePrimaryPulseWidth(1000, 75, 1, 100, 0, context));
}


void testComputePrimaryPulseWidth(void)
{
  SET_UNITY_FILENAME() {
    RUN_TEST_P(test_computePrimaryPulseWidth_basic);
    RUN_TEST_P(test_computePrimaryPulseWidth_includeAFR);
    RUN_TEST_P(test_computePrimaryPulseWidth_incorporateAFR);
    RUN_TEST_P(test_computePrimaryPulseWidth_MapMode100);
    RUN_TEST_P(test_computePrimaryPulseWidth_MapModeBaro);
    RUN_TEST_P(test_computePrimaryPulseWidth_AeAdder);
    RUN_TEST_P(test_computePrimaryPulseWidth_Nitrous);
  }  
}