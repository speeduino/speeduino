...
using randFunc = uint8_t (*)(void) noexcept;
extern randFunc rollingCutRandFunc;

struct rollingCutRandFunc_override_t
{
    rollingCutRandFunc_override_t(randFunc fn)
...
static void test_calculateFuelIgnitionChannelCut_rolling_cut_ignition_only(void)
{
    ...
    context.current.injOutputs.primary = 1;
    context.current.injOutputs.secondary = 0;
...
static void test_calculateFuelIgnitionChannelCut_rolling_cut_multi_channel_fullcut(void)
{
    ...
    context.current.injOutputs.primary = 2;
    context.current.injOutputs.secondary = 2;
    context.page6.engineProtectType = PROTECT_CUT_BOTH;
...
static void test_calculateFuelIgnitionChannelCut_pending_ignition_clears_deterministic(void)
{
    ...
    context.current.injOutputs.primary = 1;
    context.current.injOutputs.secondary = 1;
    context.page6.engineProtectType = PROTECT_CUT_BOTH;
...
static uint8_t deterministic_rand_low(void) noexcept { return 1U; }   // always triggers (< cutPercent)
static uint8_t deterministic_rand_high(void) noexcept { return 255U; } // never triggers (>= cutPercent)
static uint8_t deterministic_rand_flipper(void) noexcept
{
...
static void assert_applyRollingCutPercentage(engineProtection_test_context_t context, uint8_t expectedFuel, uint8_t expectedIgnition, SchedulerCutStatus expectedStatus)
{
    ...
    TEST_ASSERT_EQUAL_HEX8(expectedFuel, onOff.fuelChannels);
    TEST_ASSERT_EQUAL_HEX8(expectedIgnition, onOff.ignitionChannels);
    TEST_ASSERT_EQUAL(expectedStatus, onOff.status);
}
...
static void test_applyRollingCut_no_revolutions_elapsed(void)
{
    ...
    context.current.injOutputs.primary = 8;
    context.current.injOutputs.secondary = 0;
...
static void test_applyRollingCut_revolutions_elapsed_forced_no_cuts(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX8((1U << context.current.injOutputs.getTotalInjectors()) - 1U, result.fuelChannels);
...
static void test_FullCut_masks_unused(void)
{
    ...
    context.page6.engineProtectType = PROTECT_CUT_IGN;
    context.current.injOutputs.primary = 5;
    context.current.injOutputs.secondary = 0;
    ...
    context.page6.engineProtectType = PROTECT_CUT_FUEL;
    context.current.injOutputs.primary = 5;
    context.current.injOutputs.secondary = 3;
    context.current.maxIgnOutputs = 8;
    onOff = calculateFuelIgnitionChannelCut(context.current, context.page2, context.page4, context.page6, context.page9);
    TEST_ASSERT_EQUAL_HEX8(0x00, onOff.fuelChannels);
    TEST_ASSERT_EQUAL_HEX8(0xFF, onOff.ignitionChannels);
...
static void test_NoCut_masks_unused(void)
{
    ...
    context.current.injOutputs.primary = 4;
    context.current.injOutputs.secondary = 4;
    context.current.maxIgnOutputs = 8;
    auto onOff = calculateFuelIgnitionChannelCut(context.current, context.page2, context.page4, context.page6, context.page9);
    TEST_ASSERT_EQUAL_HEX8(0xFF, onOff.fuelChannels);
    TEST_ASSERT_EQUAL_HEX8(0xFF, onOff.ignitionChannels);
