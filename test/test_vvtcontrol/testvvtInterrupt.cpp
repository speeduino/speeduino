...
static void test_vvt1_off_vvt2_off(void)
{
    ...
    for (uint8_t i=0; i<LOOP_COUNT; ++i)
    {
        ...
        TEST_ASSERT_UINT16_WITHIN(1, 0, lastVvtComparatorOffset);
    }
}
...
static void test_vvt1_partial_vvt2_off(void)
{
    ...
    for (uint8_t i=0; i<LOOP_COUNT; ++i)
    {
        ...
        TEST_ASSERT_TRUE(vvtChannel2.pin.isPinLow());
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel1.targetDuty, lastVvtComparatorOffset);

        vvtInterrupt();
        // VVT1 should be turned off
        TEST_ASSERT_TRUE(vvtChannel1.pin.isPinLow());
        TEST_ASSERT_TRUE(vvtChannel2.pin.isPinLow());
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel1.maxDuty-vvtChannel1.targetDuty, lastVvtComparatorOffset);
    }
}
...
static void test_vvt1_off_vvt2_partial(void)
{
    ...
    for (uint8_t i=0; i<LOOP_COUNT; ++i)
    {
        ...
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel2.targetDuty, lastVvtComparatorOffset);

        vvtInterrupt();
        // VVT2 should be off, VVT1 should NOT activate (was at 0%)
        TEST_ASSERT_TRUE(vvtChannel1.pin.isPinLow());
        TEST_ASSERT_TRUE(vvtChannel2.pin.isPinLow());
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel2.maxDuty-vvtChannel2.targetDuty, lastVvtComparatorOffset);
    }
}
   ...
   for (uint8_t i=0; i<LOOP_COUNT; ++i)
    {
        ...
        TEST_ASSERT_TRUE(vvtChannel1.pin.isPinHigh());
        TEST_ASSERT_TRUE(vvtChannel2.pin.isPinLow());
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel1.targetDuty-vvtChannel2.targetDuty, lastVvtComparatorOffset);
   ...
   for (uint8_t i=0; i<LOOP_COUNT; ++i)
    {
        ...
        TEST_ASSERT_TRUE(vvtChannel1.pin.isPinLow());
        TEST_ASSERT_TRUE(vvtChannel2.pin.isPinHigh());
        TEST_ASSERT_UINT16_WITHIN(1, vvtChannel2.targetDuty-vvtChannel1.targetDuty, lastVvtComparatorOffset);
