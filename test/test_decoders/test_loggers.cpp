...
static void assertPrimaryTrigger(uint8_t decoder, uint8_t edge)
{
    ...
    TEST_ASSERT_BIT_HIGH_MESSAGE(BIT_DECODER_VALID_TRIGGER, decoderState, szMsg);
}
...
static void test_start_stop_rising(uint8_t decoder)
{
    ...
    assertStartStopPrimaryTrigger(decoder, getDecoderTriggerEdges().primary);
...
static void test_start_stop_ngc(void)
{
  ...
  assertPrimaryTrigger(DECODER_NGC, getDecoderTriggerEdges().primary);
