...
void setup()
{
	...
	if (configPage2.IgInv == 1)
	{
		coilHIGH = LOW, coilLOW = HIGH;
	}
	else
	{
		coilHIGH = HIGH, coilLOW = LOW;
	}
	...
	switch (configPage2.TrigPattern)
	{
		case 0:
			...
			{
				attachInterrupt(triggerInterrupt, trigger, FALLING);
			}
			break;
	...
	switch (configPage1.nCylinders)
	{
		...
		case 2:
			...
			{
				channel1InjDegrees = channel2InjDegrees = 0;
		...
		case 3:
			...
			{
				channel1InjDegrees = channel2InjDegrees = channel3InjDegrees = 0;
			} //For simultaneous, all squirts happen at the same time
			break;
		case 4:
			channel1IgnDegrees = 0;
		...
		case 8:
			...
			{
				channel1InjDegrees = channel2InjDegrees = channel3InjDegrees = channel4InjDegrees = 0;
			} //For simultaneous, all squirts happen at the same time
...
void loop()
{
	...
	switch (configPage1.mapSample)
	{
		...
		case 2:
			...
			if ((MAPcurRev == startRevolutions) || (MAPcurRev == startRevolutions + 1)) //2 revolutions are looked at for 4 stroke. 2 stroke not currently catered for.
			{
				...
				if (tempValue < MAPrunningValue)
				{
					MAPrunningValue = tempValue;
	...
	if ((mainLoopCount & 31) == 1)
	{
		...
		{
			BIT_SET(currentStatus.squirt, BIT_SQUIRT_TOOTHLOG1READY);
		}
	}
	...
	if (currentStatus.hasSync && (currentStatus.RPM > 0))
	{
		...
		if (configPage2.FixAng != 0)
		{
			currentStatus.advance = configPage2.FixAng;
		} //Check whether the user has set a fixed timing angle
		if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
		{
			currentStatus.advance = configPage2.CrankAng;
		} //Use the fixed cranking ignition angle
		...
		if (triggerToothAngle > 0 && toothHistoryIndex >= 3 && currentStatus.RPM < 3000) //toothHistoryIndex must be greater than or equal to 3 as we need the last 3 entries. Currently this mode only runs below 3000 rpm
		{
			...
			{
				angle1 = angle2 = triggerToothAngle;
			}
		...
		if (!BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
		{
			...
			if (currentStatus.PW > pwLimit)
			{
				currentStatus.PW = pwLimit;
			}
		}
		...
		if (currentStatus.RPM > ((unsigned int) (configPage2.SoftRevLim) * 100))
		{
			currentStatus.advance = configPage2.SoftLimRetard;
		} //Softcut RPM limit (If we're above softcut limit, delay timing by configured number of degrees)
		if (configPage3.launchEnabled && currentStatus.launching && (currentStatus.RPM > ((unsigned int) (configPage3.lnchSoftLim) * 100)))
		{
			currentStatus.advance = configPage3.lnchRetard;
		...
		if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
		{
			currentStatus.dwell = (configPage2.dwellCrank * 100);
		}
		else
		{
			currentStatus.dwell = (configPage2.dwellRun * 100);
		}
		...
		{
			currentStatus.dwell = divs100(currentStatus.dwell) * currentStatus.dwellCorrection;
		}
		...
		switch (configPage1.nCylinders)
		{
			...
			case 2:
				...
				if (ignition2StartAngle > 360)
				{
					ignition2StartAngle -= 360;
				}
				break;
				//3 cylinders
			case 3:
			...
			case 8:
				...
				if (ignition4StartAngle > 360)
				{
					ignition4StartAngle -= 360;
				}
				break;
		...
		if (fuelOn)
		{
			if (injector1StartAngle > crankAngle)
			{
				...
				{
					setFuelSchedule1(openInjector1and4, ((unsigned long) (injector1StartAngle - crankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector1and4);
				}
				else
				{
					setFuelSchedule1(openInjector1, ((unsigned long) (injector1StartAngle - crankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector1);
				}
			}
			...
			if (tempStartAngle > tempCrankAngle)
			{
				if (configPage1.injTiming == 1)
				{
					setFuelSchedule2(openInjector2and3, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector2and3);
				}
				else
				{
					setFuelSchedule2(openInjector2, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector2);
				}
			}
			...
			if (tempStartAngle > tempCrankAngle)
			{
				setFuelSchedule3(openInjector3, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector3);
			}
			...
			if (tempStartAngle > tempCrankAngle)
			{
				setFuelSchedule4(openInjector4, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), (unsigned long) currentStatus.PW, closeInjector4);
			}
		}
		//***********************************************************************************************
		...
		if (ignitionOn && (currentStatus.RPM < ((unsigned int) (configPage2.HardRevLim) * 100)))
		{
			...
			//if (ign1LastRev != startRevolutions)
			{
				...
				{
					ignition1StartTime = ((unsigned long) (ignition1StartAngle - crankAngle) * (unsigned long) timePerDegree);
				}
				else
				{
					ignition1StartTime = ((unsigned long) (360 - crankAngle + ignition1StartAngle) * (unsigned long) timePerDegree);
				}

				setIgnitionSchedule1(ign1StartFunction, ((unsigned long) (ignition1StartAngle - crankAngle) * (unsigned long) timePerDegree), currentStatus.dwell, ign1EndFunction);
			}
			...
			//if ( ign2LastRev != startRevolutions )
			{
				...
				if (tempStartAngle > tempCrankAngle)
				{
					ignition2StartTime = ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree);
				}
				else
				{
					ignition2StartTime = ((unsigned long) (360 - tempCrankAngle + tempStartAngle) * (unsigned long) timePerDegree);
				}

				setIgnitionSchedule2(ign2StartFunction, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), currentStatus.dwell, ign2EndFunction);
			}
			...
			if (tempStartAngle > tempCrankAngle)
			{
				setIgnitionSchedule3(ign3StartFunction, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), currentStatus.dwell, ign3EndFunction);
			}
			...
			if (tempStartAngle > tempCrankAngle)
			{
				setIgnitionSchedule4(ign4StartFunction, ((unsigned long) (tempStartAngle - tempCrankAngle) * (unsigned long) timePerDegree), currentStatus.dwell, ign4EndFunction);
			}
...
void openInjector1()
{
	...
	BIT_SET(currentStatus.squirt, BIT_SQUIRT_INJ1);
}

void closeInjector1()
{
	digitalWrite(pinInjector1, LOW);
	BIT_CLEAR(currentStatus.squirt, BIT_SQUIRT_INJ1);
}
...
{
	digitalWrite(pinCoil1, coilLOW);
	BIT_CLEAR(currentStatus.spark, 0);
}
...
void openInjector2()
{
	...
	BIT_SET(currentStatus.squirt, BIT_SQUIRT_INJ2);
} //Sets the relevant pin HIGH and changes the current status bit for injector 2 (2nd bit of currentStatus.squirt)

void closeInjector2()
{
	digitalWrite(pinInjector2, LOW);
	BIT_CLEAR(currentStatus.squirt, BIT_SQUIRT_INJ2);
}
...
{
	digitalWrite(pinCoil2, coilLOW);
	BIT_CLEAR(currentStatus.spark, 1);
}

void openInjector3()
{
	digitalWrite(pinInjector3, HIGH);
	BIT_SET(currentStatus.squirt, BIT_SQUIRT_INJ3);
} //Sets the relevant pin HIGH and changes the current status bit for injector 3 (3rd bit of currentStatus.squirt)

void closeInjector3()
{
	digitalWrite(pinInjector3, LOW);
	BIT_CLEAR(currentStatus.squirt, BIT_SQUIRT_INJ3);
}

void beginCoil3Charge()
{
	digitalWrite(pinCoil3, coilHIGH);
	BIT_SET(currentStatus.spark, 2);
	digitalWrite(pinTachOut, LOW);
}
...
{
	digitalWrite(pinCoil3, coilLOW);
	BIT_CLEAR(currentStatus.spark, 2);
}

void openInjector4()
{
	digitalWrite(pinInjector4, HIGH);
	BIT_SET(currentStatus.squirt, BIT_SQUIRT_INJ4);
} //Sets the relevant pin HIGH and changes the current status bit for injector 4 (4th bit of currentStatus.squirt)

void closeInjector4()
{
	digitalWrite(pinInjector4, LOW);
	BIT_CLEAR(currentStatus.squirt, BIT_SQUIRT_INJ4);
}

void beginCoil4Charge()
{
	digitalWrite(pinCoil4, coilHIGH);
	BIT_SET(currentStatus.spark, 3);
	digitalWrite(pinTachOut, LOW);
}
...
{
	digitalWrite(pinCoil4, coilLOW);
	BIT_CLEAR(currentStatus.spark, 3);
}
...
void openInjector1and4()
{
	digitalWrite(pinInjector1, HIGH);
	digitalWrite(pinInjector4, HIGH);
	BIT_SET(currentStatus.squirt, 0);
}
...
void closeInjector1and4()
{
	digitalWrite(pinInjector1, LOW);
	digitalWrite(pinInjector4, LOW);
	BIT_CLEAR(currentStatus.squirt, 0);
}
...
void openInjector2and3()
{
	digitalWrite(pinInjector2, HIGH);
	digitalWrite(pinInjector3, HIGH);
	BIT_SET(currentStatus.squirt, 1);
}
...
void closeInjector2and3()
{
	digitalWrite(pinInjector2, LOW);
	digitalWrite(pinInjector3, LOW);
	BIT_CLEAR(currentStatus.squirt, 1);
}
