...
volatile short toothCurrentCount = 0; //The current number of teeth (Onec sync has been achieved, this can never actually be 0
...
volatile unsigned short toothHistory[TOOTH_LOG_BUFFER];
volatile unsigned short toothHistoryIndex = 0;
...
volatile short triggerActualTeeth;
...
unsigned short triggerSecFilterTime; // The shortest time (in uS) that pulses will be accepted (Used for debounce filtering) for the secondary input
short triggerToothAngle; //The number of crank degrees that elapse per tooth
...
short toothAngles[24]; //An array for storing fixed tooth angles. Currently sized at 24 for the GM 24X decoder, but may grow later if there are other decoders that use this style
