...
void sendValues(int length)
{
  ...
  response[13] = lowByte(currentStatus.RPM); //rpm LB
  response[14] = highByte(currentStatus.RPM); //rpm HB
  ...
  int mem = freeRam();
  response[26] = lowByte(mem); //(byte)((currentStatus.loopsPerSecond >> 8) & 0xFF);
  response[27] = highByte(mem);
  
  //response[26] = lowByte(cltCalibrationTable.axisX16[0]); //(byte)((currentStatus.loopsPerSecond >> 8) & 0xFF);
  //response[27] = highByte(cltCalibrationTable.axisX16[0]);
